...
Cada método exportado por un cliente se identifica de forma única por su identificador de método. Cada cliente puede exportar hasta 32 métodos. Cada método corresponde a un conjunto de bits en el **miembro MethodType** de la estructura [**\_ RTM ENTITY \_ EXPORT \_ METHOD.**](/windows/win32/api/rtmv2/nc-rtmv2-_entity_method) Para invocar varios métodos, el cliente debe realizar una operación OR lógica de los identificadores de esos métodos. La sintaxis y semántica de las estructuras de entrada y salida de cada protocolo deben especificarse cuando se implementa el protocolo.
...
Para invocar el método de un segundo cliente, un cliente llama a la [**función RtmInvokeMethod.**](/windows/desktop/api/Rtmv2/nf-rtmv2-rtminvokemethod) El administrador de tablas de enrutamiento revoque todas las solicitudes para invocar los métodos de un cliente. El administrador de tablas de enrutamiento realiza dos funciones cuando se realiza la comunicación entre los clientes:

-   Impedir que el segundo cliente invoje un método para un cliente que está anulando el registro.
-   Mantener una solicitud de "invocación" cuando se bloquean los métodos y permitir que la solicitud continúe cuando se desbloquean los métodos.
...
Los clientes suelen bloquear métodos al realizar cambios en los datos privados que se intercambian entre clientes. Los métodos de bloqueo son una acción opcional. Un cliente también puede usar bloqueos internos para evitar que otros clientes invojen métodos.
