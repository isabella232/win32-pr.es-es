---
description: Para consumir datos específicos de eventos, el consumidor debe conocer el formato de los datos de evento.
...
ms.locfileid: "104984466"
---
# <a name="retrieving-event-data-using-mof"></a><span data-ttu-id="f5250-103">Recuperación de datos de eventos mediante MOF</span><span class="sxs-lookup"><span data-stu-id="f5250-103">Retrieving Event Data Using MOF</span></span>

<span data-ttu-id="f5250-104">Para consumir datos específicos de eventos, el consumidor debe conocer el formato de los datos de evento.</span><span class="sxs-lookup"><span data-stu-id="f5250-104">To consume event specific data, the consumer must know the format of the event data.</span></span> <span data-ttu-id="f5250-105">Si el proveedor utiliza MOF para publicar el formato de los datos de evento, puede utilizar la clase MOF para analizar los datos de evento.</span><span class="sxs-lookup"><span data-stu-id="f5250-105">If the provider used MOF to publish the format of the event data, you can use the MOF class to parse the event data.</span></span> <span data-ttu-id="f5250-106">Todos los eventos de kernel usan MOF para publicar el formato de los datos de evento.</span><span class="sxs-lookup"><span data-stu-id="f5250-106">All the kernel events use MOF to publish the format of the event data.</span></span> <span data-ttu-id="f5250-107">Para obtener información sobre la publicación de eventos, vea [publicar el esquema de eventos](publishing-your-event-schema.md).</span><span class="sxs-lookup"><span data-stu-id="f5250-107">For information on publishing events, see [Publishing Your Event Schema](publishing-your-event-schema.md).</span></span>

<span data-ttu-id="f5250-108">El análisis de los datos de eventos requiere el uso de la API de Windows Management Infrastructure (WMI).</span><span class="sxs-lookup"><span data-stu-id="f5250-108">Parsing the event data requires using the Windows Management Infrastructure (WMI) API.</span></span> <span data-ttu-id="f5250-109">El espacio de nombres ETW en el que los proveedores publican su clase MOF es \\ WMI raíz.</span><span class="sxs-lookup"><span data-stu-id="f5250-109">The ETW namespace where providers publish their MOF class is root\\wmi.</span></span> <span data-ttu-id="f5250-110">El espacio de nombres ETW contiene tres tipos de clases MOF: la clase MOF del proveedor, la clase MOF del evento y la clase MOF del tipo de evento.</span><span class="sxs-lookup"><span data-stu-id="f5250-110">The ETW namespace contains three types of MOF classes: the provider MOF class, the event MOF class, and the event type MOF class.</span></span> <span data-ttu-id="f5250-111">La clase MOF de eventos agrupa lógicamente una o más clases MOF de tipo de evento.</span><span class="sxs-lookup"><span data-stu-id="f5250-111">The event MOF class logically groups one or more event type MOF classes.</span></span> <span data-ttu-id="f5250-112">La clase de tipo de evento MOF define los datos de evento reales.</span><span class="sxs-lookup"><span data-stu-id="f5250-112">The event type MOF class defines the actual event data.</span></span>

<span data-ttu-id="f5250-113">Una clase MOF de eventos contiene un calificador de clase **GUID** cuyo valor debe coincidir con el valor del miembro **header. GUID** de la estructura de [**\_ seguimiento de eventos**](/windows/win32/api/evntrace/ns-evntrace-event_trace) .</span><span class="sxs-lookup"><span data-stu-id="f5250-113">An event MOF class contains a **Guid** class qualifier whose value must match the value in the **Header.Guid** member of the [**EVENT\_TRACE**](/windows/win32/api/evntrace/ns-evntrace-event_trace) structure.</span></span> <span data-ttu-id="f5250-114">Para asegurarse de que tiene la versión correcta de la clase, compare también el calificador de la clase **EventVersion** con el miembro **header. class. version** de la estructura de **\_ seguimiento de eventos** .</span><span class="sxs-lookup"><span data-stu-id="f5250-114">To ensure you have the correct version of the class, also compare the **EventVersion** class qualifier to the **Header.Class.Version** member of the **EVENT\_TRACE** structure.</span></span>

<span data-ttu-id="f5250-115">Después de encontrar la clase de eventos correcta, enumere sus clases de tipo de evento secundario para buscar la clase que contiene el formato de los datos de evento.</span><span class="sxs-lookup"><span data-stu-id="f5250-115">After you find the correct event class, enumerate its child event type classes to find the class that contains the format of the event data.</span></span> <span data-ttu-id="f5250-116">La clase de tipo de evento correcta contiene un calificador de clase **EventType** cuyo valor coincide con el valor del miembro **header. class. Type** de la estructura de [**\_ seguimiento de eventos**](/windows/win32/api/evntrace/ns-evntrace-event_trace) .</span><span class="sxs-lookup"><span data-stu-id="f5250-116">The correct event type class contains an **EventType** class qualifier whose value matches the value in the **Header.Class.Type** member of the [**EVENT\_TRACE**](/windows/win32/api/evntrace/ns-evntrace-event_trace) structure.</span></span>

<span data-ttu-id="f5250-117">Después, puede usar la API de WMI para enumerar las propiedades de la clase MOF.</span><span class="sxs-lookup"><span data-stu-id="f5250-117">You can then use the WMI API to enumerate the properties of the MOF class.</span></span> <span data-ttu-id="f5250-118">Use los calificadores y el tipo de datos de cada propiedad para determinar el tamaño del elemento de datos en los datos de evento que se van a leer y cómo darles formato.</span><span class="sxs-lookup"><span data-stu-id="f5250-118">Use the qualifiers and data type of each property to determine the size of the data element in the event data to read and how to format it.</span></span> <span data-ttu-id="f5250-119">Para obtener una lista de los calificadores MOF que admite ETW, consulte [calificadores MOF de seguimiento de eventos](event-tracing-mof-qualifiers.md).</span><span class="sxs-lookup"><span data-stu-id="f5250-119">For a list of MOF qualifiers that ETW supports, see [Event Tracing MOF Qualifiers](event-tracing-mof-qualifiers.md).</span></span>

<span data-ttu-id="f5250-120">Dado que ETW no fuerza una alineación entre los valores de los datos de evento, conversión o la asignación del valor directamente desde un búfer puede producir un error de alineación. no debe crear una estructura a partir de la clase MOF e intentar utilizarla para consumir datos de eventos.</span><span class="sxs-lookup"><span data-stu-id="f5250-120">Because ETW does not force an alignment between event data values, typecasting or assigning the value directly from a buffer may cause an alignment fault; you should not create a structure from the MOF class and try to use it to consume event data.</span></span> <span data-ttu-id="f5250-121">Por ejemplo, si tiene un carácter seguido de ULONGLONG, el ULONGLONG no se alineará con un límite de 8 bytes, por lo que una asignación produciría una excepción de alineación.</span><span class="sxs-lookup"><span data-stu-id="f5250-121">For example, if you have a character followed by ULONGLONG, the ULONGLONG would not be aligned to an 8-byte boundary, so an assignment would cause an alignment exception.</span></span> <span data-ttu-id="f5250-122">(En equipos de 64 bits, esto ocurre con más frecuencia). Por esta razón, debe utilizar CopyMemory para copiar los datos del búfer en una variable local.</span><span class="sxs-lookup"><span data-stu-id="f5250-122">(On 64-bit computers, this happens more often.) For this reason, you should use CopyMemory to copy the data from the buffer to a local variable.</span></span> <span data-ttu-id="f5250-123">Además, si se revisa el evento posteriormente, es posible que el consumidor no funcione si se intenta usar una estructura.</span><span class="sxs-lookup"><span data-stu-id="f5250-123">Also, if the event is later revised, your consumer may not work if you try to use a structure.</span></span>

<span data-ttu-id="f5250-124">A partir de Windows Vista, se recomienda usar las funciones de la aplicación auxiliar de datos de seguimiento (TDH) para consumir eventos que se publicaron mediante clases MOF.</span><span class="sxs-lookup"><span data-stu-id="f5250-124">Beginning with Windows Vista, you are encouraged to use the trace data helper (TDH) functions to consume events that were published using MOF classes.</span></span> <span data-ttu-id="f5250-125">Para obtener más información, consulte [recuperar datos de eventos mediante TDH](retrieving-event-data-using-tdh.md).</span><span class="sxs-lookup"><span data-stu-id="f5250-125">For details, see [Retrieving Event Data Using TDH](retrieving-event-data-using-tdh.md).</span></span>

<span data-ttu-id="f5250-126">En el ejemplo siguiente se muestra cómo consumir eventos definidos por una clase MOF.</span><span class="sxs-lookup"><span data-stu-id="f5250-126">The following example shows how to consume events that are defined by a MOF class.</span></span>
