...
---
# <a name="cim_diskpartition-class"></a>\_Clase DiskPartition de CIM

La clase **CIM \_ DiskPartition** representa un intervalo contiguo de bloques lógicos que el sistema operativo identifica mediante el tipo de la partición y los campos de subtipo. Las particiones de disco deben realizarse directamente mediante medios físicos (indicados por la Asociación [**\_ RealizesDiskPartition de CIM**](cim-realizesdiskpartition.md) ) o basados en volúmenes de almacenamiento.
...
La clase **CIM \_ DiskPartition** tiene estos tipos de miembros:
...
La clase **CIM \_ DiskPartition** tiene estos métodos.



| Método                                                                   | Descripción                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-diskpartition.md)                 | Solicita un restablecimiento del dispositivo lógico. No implementado por WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-diskpartition.md) | Define el estado de energía deseado para un dispositivo lógico y cuándo debe colocarse un dispositivo en ese estado. No implementado por WMI.<br/> |
...
La clase **CIM \_ DiskPartition** tiene estas propiedades.
...
Especifica si los medios se pueden leer, escribir o ambos, por ejemplo. Esta propiedad se hereda de [**\_ StorageExtent CIM**](cim-storageextent.md).
...
Compatibilidad con lectura y escritura.
...
Tamaño, en bytes, de los bloques que forman la extensión del almacenamiento. Si es un tamaño de bloque variable, se debe especificar el tamaño máximo de bloque, en bytes. Si se desconoce el tamaño de bloque, o si un concepto de bloque no es válido (por ejemplo, para las extensiones de agregado, la memoria o los discos lógicos), escriba el valor 1.
...
Si es **true**, la partición de disco se etiqueta como de arranque. Esto no significa que se cargue un sistema operativo en la partición.
...
Si es **true**, la partición de disco se etiqueta como la partición principal para un equipo.
...
La clase **CIM \_ DiskPartition** se deriva de [**\_ StorageExtent de CIM**](cim-storageextent.md).

WMI no implementa esta clase. Para las clases derivadas de **CIM \_ DiskPartition**, vea [clases Win32](win32-provider.md).
