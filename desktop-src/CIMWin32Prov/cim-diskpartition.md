---
description: La clase CIM DiskPartition representa un intervalo contiguo de bloques lógicos que el sistema operativo puede identificar mediante los campos de tipo y subtipo de \_ la partición.
...
La **clase \_ DiskPartition de CIM** tiene estos tipos de miembros:
...
La **clase \_ DiskPartition de CIM** tiene estos métodos.
...
| [**SetPowerState**](setpowerstate-method-in-class-cim-diskpartition.md) | Define el estado de energía deseado para un dispositivo lógico y cuándo se debe colocar un dispositivo en ese estado. No implementado por WMI.<br/> |
...
La **clase \_ DiskPartition de CIM** tiene estas propiedades.
...
Especifica si el medio es legible, se puede escribir o ambos, por ejemplo. Esta propiedad se hereda de [**CIM \_ StorageExtent.**](cim-storageextent.md)
...
Se admite la lectura y escritura.
...
Tamaño, en bytes, de los bloques que forman la extensión de almacenamiento. Si es un tamaño de bloque variable, se debe especificar el tamaño máximo del bloque, en bytes. Si se desconoce el tamaño del bloque o si un concepto de bloque no es válido (por ejemplo, para extensiones de agregado, memoria o discos lógicos), escriba el valor 1.
...
Si **es TRUE,** la partición de disco se etiqueta como la partición principal de un sistema informático.
...
La **clase \_ DiskPartition** de CIM se deriva de [**Cim \_ StorageExtent.**](cim-storageextent.md)
