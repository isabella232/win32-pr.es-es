...
---
# <a name="cim_protectedspaceextent-class"></a>CIM \_ ProtectedSpaceExtent (clase)

La clase **CIM \_ ProtectedSpaceExtent** representa direcciones de bloques lógicos direccionables, que se tratan como una única extensión de almacenamiento, pero se encuentran en una única extensión física. Las extensiones de espacio protegido excluyen los bloques lógicos asignados como datos de comprobación y contienen información de asignación de profundidad de franjas de datos de usuario. Una posibilidad alternativa, si se utiliza la configuración automática, es crear una instancia de la [**clase \_ AggregatePSExtent de CIM**](cim-aggregatepsextent.md) o extenderla.
...
La clase **CIM \_ ProtectedSpaceExtent** tiene estos tipos de miembros:
...
La clase **CIM \_ ProtectedSpaceExtent** tiene estos métodos.



| Método                                                                          | Descripción                                                                                                                              |
|:--------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-protectedspaceextent.md)                 | Solicita un restablecimiento del dispositivo lógico. No implementado por WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-protectedspaceextent.md) | Define el estado de energía deseado para un dispositivo lógico y cuándo debe colocarse un dispositivo en ese estado. No implementado por WMI.<br/> |
...
La clase **CIM \_ ProtectedSpaceExtent** tiene estas propiedades.
...
Tamaño de bloque, en bytes, de los bloques que forman la extensión de almacenamiento. Si el tamaño de bloque es variable, se debe especificar el tamaño máximo de bloque, en bytes. Si se desconoce el tamaño de bloque, o si un concepto de bloque no es válido (por ejemplo, para las extensiones de agregado, la memoria o los discos lógicos), escriba 1 (uno).
...
Calificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Extensión de espacio protegido \| de DMTF 001,6 "), [**unidades**](/windows/desktop/WmiSdk/standard-qualifiers) (" bytes ")
</dt> </dl>

En el caso de una clase de **CIM \_ ProtectedSpaceExtent** que se asigna a una clase [**\_ VolumeSet de CIM**](cim-volumeset.md) , esta propiedad es el número de bytes de datos de usuario que se colocan en la extensión de espacio protegido antes de pasar a la siguiente extensión de espacio protegido en el conjunto de volúmenes. De lo contrario, se considera que esta extensión de espacio protegido está sin asignar y esta propiedad debe establecerse en 0.
...
La clase **CIM \_ ProtectedSpaceExtent** se deriva de [**\_ StorageExtent CIM**](cim-storageextent.md).
