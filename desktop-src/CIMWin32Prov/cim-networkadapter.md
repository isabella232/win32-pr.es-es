...
---
# <a name="cim_networkadapter-class"></a>CIM ( \_ clase)

La **clase \_ CIM** Networking es una clase abstracta que define los conceptos generales de hardware de red (por ejemplo, la dirección permanente o la velocidad de funcionamiento). La información se transmite mediante la Asociación [**\_ DeviceSAPImplementation de CIM**](cim-devicesapimplementation.md) .

Los adaptadores de red y los puntos de conexión relacionados representan el potencial de conectividad entre los equipos del mismo nivel. La posibilidad de conectividad es significativamente diferente de las relaciones maestra, subordinada y controlada por del [**\_ controlador CIM**](cim-controller.md). Sin embargo, en ocasiones, un único dispositivo actúa como adaptador de red y como controlador, por ejemplo, cuando un adaptador de canal de fibra funciona como controlador SCSI de un equipo. En ese caso, los aspectos del dispositivo están orientados a la red y otros están orientados a los controladores. Se debe crear una instancia de las clases de controlador y adaptador y también se debe crear una relación de identidad de dispositivo para vincular los distintos aspectos del dispositivo.
...
La clase **CIM \_ adaptador** de tipo tiene estos tipos de miembros:
...
La clase **CIM \_ adaptador** de la clase tiene estos métodos.



| Método                                                                    | Descripción                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Reset**](reset-method-in-class-cim-networkadapter.md)                 | Solicita un restablecimiento del dispositivo lógico. No implementado por WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | Define el estado de energía deseado para un dispositivo lógico y cuándo debe colocarse un dispositivo en ese estado. No implementado por WMI.<br/> |
...
La clase **CIM \_ adaptador** de clase tiene estas propiedades.
...
Si **es true**, el adaptador de red puede determinar automáticamente la velocidad u otras características de comunicación del medio de red conectado.
...
Velocidad máxima, en bits por segundo (BPS), del adaptador de red.
...
Lista de direcciones de red de un adaptador.
...
Dirección de red codificada de forma rígida en un adaptador. La dirección codificada de forma rígida se puede cambiar con una actualización de firmware o una configuración de software. Si se cambia, el campo debe actualizarse cuando se realiza el cambio. Esta propiedad debe dejarse en blanco si no existe ninguna dirección codificada de forma rígida para el adaptador de red.
...
Estimación del ancho de banda actual, en BPS. En el caso de los puntos de conexión que varían en ancho de banda o para aquellos en los que no se puede realizar una estimación precisa, esta propiedad debe contener el ancho de banda nominal.
...
La clase de **\_ adaptador** de la CIM se deriva del [**\_ LogicalDevice de CIM**](cim-logicaldevice.md).

WMI no implementa esta clase. Para las clases que se derivan de la **\_ adaptador** de la clase CIM, vea [clases Win32](win32-provider.md).
