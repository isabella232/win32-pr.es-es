...
La clase **CIM \_ DiskDrive** tiene estos tipos de miembros:
...
La clase **CIM \_ DiskDrive** tiene estos métodos.
...
| [**SetPowerState**](setpowerstate-method-in-class-cim-cdromdrive.md) | Define el estado de energía deseado para un dispositivo lógico y cuándo debe colocarse un dispositivo en ese estado. No implementado por WMI.<br/> |
...
La clase **CIM \_ DiskDrive** tiene estas propiedades.
...
Se sabe que el dispositivo está en modo de ahorro de energía, pero su estado exacto en este modo es desconocido.
...
El dispositivo no funciona, pero puede pasarse a la potencia completa "rápidamente".
...
El dispositivo está en un estado de advertencia y también en un modo de ahorro de energía.
...
Capacidades del dispositivo de acceso a medios. Esta propiedad se hereda de [**\_ MediaAccessDevice CIM**](cim-mediaaccessdevice.md).
...
Distingue un dispositivo que puede tener acceso a ambos lados de un medio de dos caras desde un dispositivo que lee solo un lado y requiere que el medio se desactive.
...
Indica que el medio no tiene que extraerse explícitamente del dispositivo antes de que un elemento selector tenga acceso a él.
...
Matriz de cadenas de formato libre que proporcionan explicaciones detalladas para las características del dispositivo de acceso indicadas en la matriz de **funcionalidades** . Esta propiedad se hereda de [**\_ MediaAccessDevice CIM**](cim-mediaaccessdevice.md).

> [!Note]  
> Cada entrada de esta matriz está relacionada con la entrada de la matriz de **capacidades** que se encuentra en el mismo índice.
...
Si es **true**, el error comunicado en la propiedad **LastErrorCode** está desactivado.
...
Cadena de forma libre que describe el tipo de detección y corrección de errores admitidos por el dispositivo.
...
Tamaño máximo, en kilobytes, de los medios admitidos por el dispositivo. Los kilobytes se interpretan como el número de bytes multiplicado por 1000 (no por el número de bytes multiplicado por 1024).
...
Si **es true**, el dispositivo de acceso a medios necesita limpieza. Si la limpieza manual o automática es posible, se indica en la propiedad array de **funcionalidades** .
...
Cuando el dispositivo de acceso a medios admite varios medios individuales, esta propiedad define el número máximo que se puede admitir o insertar.
...
El método [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) se puede invocar con el parámetro *PowerState* establecido en 5 ("ciclo de energía") y el parámetro *Time* establecido en una fecha y hora específicas, o Interval, para el encendido.
...
La clase **CIM \_ DiskDrive** se deriva de [**\_ MediaAccessDevice de CIM**](cim-mediaaccessdevice.md).

WMI no implementa esta clase. Vea [clases Win32](win32-provider.md) para clases derivadas de **CIM \_ DiskDrive**.
