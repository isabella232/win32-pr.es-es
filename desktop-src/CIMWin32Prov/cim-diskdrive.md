---
description: La clase \_ Cim DiskDrive representa una unidad de disco físico tal como la ve el sistema operativo.
ms.assetid: 3a63506e-455e-4108-b0c7-03b2af249d61
ms.tgt_platform: multiple
title: CIM_DiskDrive clase (proveedores WMI CIMWin32)
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "119080759"
---
# <a name="cim_diskdrive-class-cimwin32-wmi-providers"></a>CIM_DiskDrive clase (proveedores WMI CIMWin32)

La **clase \_ Cim DiskDrive** representa una unidad de disco físico tal como la ve el sistema operativo. Las características de la unidad de disco corresponden a las características lógicas y de administración de la unidad y, en algunos casos, pueden no reflejar las características físicas del dispositivo. Una interfaz a una unidad física es miembro de esta clase. Sin embargo, un objeto basado en otro dispositivo lógico no es miembro de esta clase.
...
La **clase \_ Cim DiskDrive** tiene estos tipos de miembros:
...
La **clase \_ DiskDrive de CIM** tiene estos métodos.



| Método                                                                | Descripción                                                                                                                              |
|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Restablecer**](reset-method-in-class-cim-cdromdrive.md)                 | Solicita un restablecimiento del dispositivo lógico. Wmi no implementa .<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-cdromdrive.md) | Define el estado de energía deseado para un dispositivo lógico y cuándo se debe colocar un dispositivo en ese estado. Wmi no implementa .<br/> |
...
La **clase \_ Cim DiskDrive** tiene estas propiedades.
...
Se sabe que el dispositivo está en modo de ahorro de energía, pero se desconoce su estado exacto en este modo.
...
El dispositivo está en estado de advertencia y también en modo de ahorro de energía.
...
Funcionalidades del dispositivo de acceso multimedia. Esta propiedad se hereda de [**CIM \_ MediaAccessDevice.**](cim-mediaaccessdevice.md)
...
Distingue un dispositivo que puede tener acceso a ambos lados de los medios de doble cara de un dispositivo que solo lee un solo lado y requiere que el medio se deba cambiar.
...
Indica que no es necesario expulsar explícitamente el medio del dispositivo antes de que un elemento selector tenga acceso a él.
...
Matriz de cadenas de forma libre que proporcionan explicaciones detalladas de las características del dispositivo de acceso indicadas en la **matriz Capabilities.** Esta propiedad se hereda de [**CIM \_ MediaAccessDevice.**](cim-mediaaccessdevice.md)
...
Nombre de la clase (o subclase) utilizada en la creación de una instancia de . Cuando se usa con otras propiedades clave de la clase , esta propiedad permite identificar de forma única todas las instancias de la clase y sus subclases.
...
Si **es TRUE,** se borra el error notificado **en la propiedad LastErrorCode.**
...
Cadena de forma libre que describe el tipo de detección y corrección de errores que admite el dispositivo.
...
Tamaño máximo, en kilobytes, de los medios admitidos por el dispositivo. Los kilobytes se interpretan como el número de bytes multiplicado por 1000 (no el número de bytes multiplicado por 1024).
...
Si **es TRUE,** el dispositivo de acceso multimedia debe limpiarse. Si la limpieza manual o automática es posible se indica en la propiedad **de matriz Capabilities.**
...
Cuando el dispositivo de acceso multimedia admite varios medios individuales, esta propiedad define el número máximo que puede ser compatible o insertado.
...
Se [**admite el método SetPowerState.**](setpowerstate-method-in-class-cim-diskdrive.md)
...
El [**método SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) se puede invocar con el *parámetro PowerState* establecido en 5 ("Ciclo de energía").
...
El [**método SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) se puede invocar con el parámetro *PowerState* establecido en 5 ("Ciclo de energía") y el parámetro *Time* establecido en una fecha y hora específicas, o un intervalo, para el encendido.
...
La **clase \_ Cim DiskDrive** se deriva de [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).

WMI no implementa esta clase. Vea [Clases win32 para](win32-provider.md) obtener clases derivadas de CIM **\_ DiskDrive.**
