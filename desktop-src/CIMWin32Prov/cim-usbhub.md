---
description: La clase \_ CIM USBHub representa las funcionalidades y la administración de un concentrador USB.
...
ms.locfileid: "127062735"
---
# <a name="cim_usbhub-class"></a>Cim \_ USBHub (clase)

La **clase \_ CIM USBHub** representa las funcionalidades y la administración de un concentrador USB.
...
La **clase \_ CIM USBHub** tiene estos tipos de miembros:
...
La **clase \_ CIM USBHub** tiene estos métodos.



| Método                                                            | Descripción                                                                                                                              |
|:------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**GetDescriptor**](getdescriptor-method-in-class-cim-usbhub.md) | Devuelve el descriptor del dispositivo USB. Wmi no implementa .<br/>                                                                    |
| [**Reset**](reset-method-in-class-cim-usbhub.md)                 | Solicita un restablecimiento del dispositivo lógico. Wmi no implementa .<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-usbhub.md) | Define el estado de energía deseado para un dispositivo lógico y cuándo se debe colocar un dispositivo en ese estado. Wmi no implementa .<br/> |
...
La **clase \_ CIM USBHub** tiene estas propiedades.
...
Esta propiedad se hereda de [**CIM \_ USBDevice.**](cim-usbdevice.md)
...
Calificadores: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ USBDevice**](cim-usbdevice.md).**CurrentConfigValue**")
</dt> </dl>

Configuración alternativa USB para cada interfaz de la configuración seleccionada actualmente (indicada por la **propiedad CurrentConfigValue).** Esta matriz tiene una entrada para cada interfaz de la configuración. Si la **propiedad CurrentConfigValue** tiene un valor de 0 (cero), lo que indica que el dispositivo no está configurado, la matriz no está definida. Para obtener más información sobre el análisis de esta cadena de octeto, consulte la especificación USB.
...
Calificadores: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ USBDevice**](cim-usbdevice.md).**CurrentAlternateSettings**")
</dt> </dl>

Configuración seleccionada actualmente para el dispositivo. Si este valor es 0 (cero), el dispositivo no está configurado.
...
Si **es TRUE,** la energía se cambia a todos los puertos del concentrador simultáneamente. Si **es FALSE,** la energía se cambia individualmente para cada puerto.
...
Número de puertos de bajada en el centro, incluidos los insertados en el silicio del centro.
...
Versión USB más reciente compatible con el dispositivo USB. Esta propiedad se expresa como un decimal codificado binariamente (BCD) donde un separador decimal está implícito entre el segundo y el tercer dígito. Por ejemplo, un valor de 0x201 indica que se admite la versión 2.01.
...
La **clase \_ CIM USBHub** se deriva de [**CIM \_ USBDevice**](cim-usbdevice.md).

WMI no implementa esta clase. Para las clases WMI derivadas de **\_ CIM USBHub,** vea [Clases Win32](win32-provider.md).
