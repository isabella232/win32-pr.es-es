...
---
# <a name="cim_discretesensor-class"></a>\_Clase DiscreteSensor de CIM

La clase **CIM \_ DiscreteSensor** tiene un conjunto de valores de cadena válidos que puede notificar. Los valores se enumeran en la propiedad **PossibleValues** del sensor. Un sensor discreto siempre tiene una lectura actual que corresponde a uno de los valores enumerados.

Dada la adición de las propiedades **CurrentState** y **PossibleStates** al [**\_ sensor CIM**](cim-sensor.md), la subclase del sensor discreto ya no es necesaria; sin embargo, se conserva por compatibilidad con versiones anteriores. La información de las propiedades **CurrentReading** y **PossibleValues** normalmente tiene los mismos valores y semánticas que para las propiedades **CurrentState** y **PossibleStates** , que se heredan del **\_ sensor CIM**.
...
Nombre de la clase o la subclase utilizada en la creación de una instancia de. Cuando se usa con otras propiedades de clave de la clase, esta propiedad permite que todas las instancias de la clase y sus subclases se identifiquen de forma única.
