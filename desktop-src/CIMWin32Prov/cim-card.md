...
Si **es TRUE**, para funcionar correctamente, se requiere al menos un placa o una tarjeta auxiliar.
...
Si **es TRUE,** la tarjeta es físicamente única de otras tarjetas del mismo tipo y, por lo tanto, requiere una ranura especial. Por ejemplo, una tarjeta de doble ancho requiere dos ranuras. Otro ejemplo es cuando se puede usar una tarjeta determinada para la misma función general que otras tarjetas, pero requiere una ranura especial (extra larga, por ejemplo); mientras que otras tarjetas se pueden colocar en cualquier ranura disponible. Si **es TRUE,** la **propiedad RequirementsDescription** correspondiente debe especificar la naturaleza de la unidad o el propósito de la tarjeta.
...
WMI no implementa esta clase. Para obtener más información sobre las clases derivadas de la **\_ tarjeta CIM**, vea [Clases Win32](win32-provider.md).
