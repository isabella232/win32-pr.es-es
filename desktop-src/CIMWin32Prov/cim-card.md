...
La clase **CIM \_ Card** tiene estos tipos de miembros:
...
| Método                                                        | Descripción                                                                                                                                    |
|:--------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------|
| [**IsCompatible**](iscompatible-method-in-class-cim-card.md) | Comprueba si el elemento físico al que se hace referencia puede incluirse o insertarse en el paquete físico. No implementado por WMI.<br/> |
...
La clase **CIM \_ Card** tiene estas propiedades.
...
Si es **true**, esta tarjeta es una placa base o, más genéricamente, una placa base en un chasis.
...
Calificadores: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**\_ tarjeta CIM**.**SpecialRequirements**")
</dt> </dl>

Cadena de forma libre que describe las formas en que la tarjeta es única físicamente desde otras tarjetas. Esta propiedad solo tiene significado cuando la propiedad booleana correspondiente, **SpecialRequirements**, se establece en **true**.
...
Si es **true**, para que funcione correctamente, se requiere al menos una tarjeta daughterboard o auxiliar.
...
Cadena de forma libre que describe el posicionamiento de la ranura, el uso típico, las restricciones, el espaciado de ranura individual u otra información pertinente para las ranuras de una tarjeta.
...
Calificadores: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("**\_ tarjeta CIM**.**RequirementsDescription**")
</dt> </dl>

Si es **true**, la tarjeta es única físicamente desde otras tarjetas del mismo tipo y, por lo tanto, requiere una ranura especial. Por ejemplo, una tarjeta de doble ancho requiere dos ranuras. Otro ejemplo es cuando se puede usar una determinada tarjeta para la misma función general que otras tarjetas, pero requiere una ranura especial (extra larga, por ejemplo); sin embargo, se pueden colocar otras tarjetas en cualquier ranura disponible. Si **es true**, la propiedad **RequirementsDescription** correspondiente debe especificar la naturaleza de la unicidad o el propósito de la tarjeta.
...
La clase **CIM \_ Card** se deriva de [**CIM \_ PhysicalPackage**](cim-physicalpackage.md).

WMI no implementa esta clase. Para obtener más información acerca de las clases derivadas de la **\_ tarjeta CIM**, vea [clases Win32](win32-provider.md).
