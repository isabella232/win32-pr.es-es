---
description: Permite que el componente de servidor de una aplicación de transporte establezca un contexto de [*seguridad*](../secgloss/s-gly.md) entre el servidor y un cliente remoto que usa Schannel.
...
Puntero a una [estructura CtxtHandle.](sspi-handles.md) En la primera llamada a **AcceptSecurityContext (Schannel),** este puntero es **NULL.** En las llamadas posteriores, *phContext* es el identificador del contexto parcialmente formado que la primera llamada devolvió en el parámetro *phNewContext.*
...
2.  El servidor llama **a AcceptSecurityContext (Schannel)** la primera vez, lo que genera un token de respuesta que se envía al cliente.
3.  El cliente recibe el token y lo pasa a [**InitializeSecurityContext (Schannel).**](initializesecuritycontext--schannel.md) Si **InitializeSecurityContext (Schannel)** devuelve SEC E OK, se ha completado la autenticación mutua y se \_ puede iniciar una sesión \_ segura. Si **InitializeSecurityContext (Schannel)** devuelve un código de error, finaliza la negociación de autenticación mutua. De lo contrario, el token de seguridad devuelto por **InitializeSecurityContext (Schannel)** se envía al cliente y se repiten los pasos 2 y 3.
