---
description: Permite que el componente de servidor [](../secgloss/s-gly.md) de una aplicación de transporte establezca un contexto de seguridad entre el servidor y un cliente remoto que usa NTLM.
...
Identificador de las credenciales del servidor. El servidor llama a la función [**AcquireCredentialsHandle (NTLM)**](acquirecredentialshandle--ntlm.md) con la marca SECPKG CRED INBOUND o SECPKG CRED BOTH establecida para \_ \_ recuperar este \_ \_ identificador.
...
Puntero a una [estructura CtxtHandle.](sspi-handles.md) En la primera llamada a **AcceptSecurityContext (NTLM),** este puntero es **NULL.** En las llamadas posteriores, *phContext* es el identificador del contexto con formato parcial que la primera llamada devolvió en el parámetro *phNewContext.*
...
2.  El servidor llama **a AcceptSecurityContext (NTLM)** la primera vez, lo que genera un token de respuesta que se envía al cliente.
