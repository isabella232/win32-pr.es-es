...
ms.locfileid: "120015925"
---
# <a name="initializesecuritycontext-general-function"></a>Función InitializeSecurityContext (General)

La **función InitializeSecurityContext (General)** inicia el contexto de seguridad de salida del lado cliente [*desde*](../secgloss/s-gly.md) un identificador de credenciales. La función se usa para crear un contexto [*de seguridad entre*](../secgloss/s-gly.md) la aplicación cliente y un par remoto. **InitializeSecurityContext (General)** devuelve un token que el cliente debe pasar al elemento remoto del mismo nivel, que a su vez envía el elemento del mismo nivel a la implementación de seguridad local a través de la llamada [**AcceptSecurityContext (General).**](acceptsecuritycontext--general.md) Todos los llamadores deben considerar opaco el token generado.
...
Este parámetro no se usa con Digest o Schannel. Esta establece en cero.
...
Al usar el Microsoft Digest SSP, este parámetro recibe la respuesta de desafío que se debe enviar al servidor.
...
No hay ninguna hora de expiración para Microsoft Digest o credenciales del [*contexto*](../secgloss/s-gly.md)de seguridad de SSP.
...
Si el servidor ha respondido correctamente, el [*paquete*](../secgloss/s-gly.md) de seguridad devuelve SEC E \_ OK y se establece una sesión \_ segura.
