...
<span id="_security_handle_gly"></span><span id="_SECURITY_HANDLE_GLY"></span>**asa**
</dt> <dd>

Token que se usa para identificar o tener acceso a un objeto, como el identificador de un proveedor de servicios criptográficos, un almacén de certificados, un mensaje o un par de claves.

</dd> <dt>

<span id="_security_hash_gly"></span><span id="_SECURITY_HASH_GLY"></span>**LM**
</dt> <dd>

Resultado de tamaño fijo obtenido al aplicar una función matemática (el *algoritmo hash*) a una cantidad arbitraria de datos. (También conocido como "síntesis del mensaje").

Vea también *funciones de hash*.
...
Objeto que se usa para aplicar un algoritmo hash a los mensajes o claves de sesión. El objeto hash se crea mediante una llamada a **CryptCreateHash**. La definición del objeto se define mediante el CSP especificado en la llamada.
...
<span id="_security_hashing_functions_gly"></span><span id="_SECURITY_HASHING_FUNCTIONS_GLY"></span>**funciones de hash**
</dt> <dd>
...
<span id="_security_hash_based_message_authentication_code_gly"></span><span id="_SECURITY_HASH_BASED_MESSAGE_AUTHENTICATION_CODE_GLY"></span>**Código de autentificación de mensajes (MAC) basado en hash**
</dt> <dd>

HMAC Algoritmo hash con clave simétrica implementado por proveedores de servicios criptográficos de Microsoft. Un HMAC se usa para comprobar la integridad de los datos con el fin de garantizar que no se han modificado durante el almacenamiento o el tránsito. Se puede usar con cualquier algoritmo hash criptográfico iterado, como MD5 o SHA-1. CryptoAPI hace referencia a este algoritmo por su identificador de algoritmo (CALG \_ HMAC) y la clase ( \_ hash de clase alg \_ ).
...
Vea *código de autentificación de mensajes (Mac) basado en hash*.
