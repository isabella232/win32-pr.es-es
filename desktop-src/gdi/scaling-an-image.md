...
ms.locfileid: "104082511"
---
# <a name="scaling-an-image"></a><span data-ttu-id="d219f-104">Escalado de una imagen</span><span class="sxs-lookup"><span data-stu-id="d219f-104">Scaling an Image</span></span>

<span data-ttu-id="d219f-105">Algunas aplicaciones escalan imágenes; es decir, muestran vistas ampliadas o reducidas de una imagen.</span><span class="sxs-lookup"><span data-stu-id="d219f-105">Some applications scale images; that is, they display zoomed or reduced views of an image.</span></span> <span data-ttu-id="d219f-106">Por ejemplo, una aplicación de dibujo puede proporcionar una característica de zoom que permite al usuario ver y editar un dibujo píxel a píxel.</span><span class="sxs-lookup"><span data-stu-id="d219f-106">For example, a drawing application may provide a zoom feature that enables the user to view and edit a drawing on a pixel-by-pixel basis.</span></span>

<span data-ttu-id="d219f-107">Las aplicaciones escalan imágenes mediante una llamada a la función [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .</span><span class="sxs-lookup"><span data-stu-id="d219f-107">Applications scale images by calling the [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) function.</span></span> <span data-ttu-id="d219f-108">Al igual que la función [**bitblt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) , **StretchBlt** copia los datos de mapa de bits de un mapa de bits en un contexto de dispositivo de origen ([**DC**](/windows/desktop/api/Winuser/nf-winuser-getdcex)) en un mapa de bits en un controlador de dominio de destino.</span><span class="sxs-lookup"><span data-stu-id="d219f-108">Like the [**BitBlt**](/windows/desktop/api/Wingdi/nf-wingdi-bitblt) function, **StretchBlt** copies bitmap data from a bitmap in a source device context ([**DC**](/windows/desktop/api/Winuser/nf-winuser-getdcex)) into a bitmap in a target DC.</span></span> <span data-ttu-id="d219f-109">Sin embargo, a diferencia de la función **bitblt** , **StretchBlt** escala la imagen basándose en las dimensiones especificadas de los rectángulos de origen y de destino.</span><span class="sxs-lookup"><span data-stu-id="d219f-109">However, unlike the **BitBlt** function, **StretchBlt** scales the image based on the specified dimensions of the source and target rectangles.</span></span> <span data-ttu-id="d219f-110">Si el rectángulo de origen es mayor que el rectángulo de destino, parecerá que la imagen resultante se ha reducido. Si el rectángulo de origen es menor que el rectángulo de destino, parecerá que la imagen resultante se ha expandido.</span><span class="sxs-lookup"><span data-stu-id="d219f-110">If the source rectangle is larger than the target rectangle, the resultant image will appear to have shrunk; if the source rectangle is smaller than the target rectangle, the resultant image will appear to have expanded.</span></span>

<span data-ttu-id="d219f-111">Si el rectángulo de destino es menor que el rectángulo de origen, [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) quita los datos de color de la imagen según un modo de ajuste especificado, como se muestra en la tabla siguiente.</span><span class="sxs-lookup"><span data-stu-id="d219f-111">If the target rectangle is smaller than the source rectangle, [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) removes color data from the image according to a specified stretch mode as shown in the following table.</span></span>



| <span data-ttu-id="d219f-112">Modo de stretch</span><span class="sxs-lookup"><span data-stu-id="d219f-112">Stretch mode</span></span> | <span data-ttu-id="d219f-113">Método</span><span class="sxs-lookup"><span data-stu-id="d219f-113">Method</span></span>                                                                                                                    |
|--------------|---------------------------------------------------------------------------------------------------------------------------|
| <span data-ttu-id="d219f-114">BLACKONWHITE</span><span class="sxs-lookup"><span data-stu-id="d219f-114">BLACKONWHITE</span></span> | <span data-ttu-id="d219f-115">Realiza una operación AND lógica en los datos de color de los píxeles eliminados y los datos de color de los píxeles restantes.</span><span class="sxs-lookup"><span data-stu-id="d219f-115">Performs a logical AND operation on the color data for the eliminated pixels and the color data for the remaining pixels.</span></span> |
| <span data-ttu-id="d219f-116">WHITEONBLACK</span><span class="sxs-lookup"><span data-stu-id="d219f-116">WHITEONBLACK</span></span> | <span data-ttu-id="d219f-117">Realiza una operación OR lógica en los datos de color de los píxeles eliminados y los datos de color de los píxeles restantes.</span><span class="sxs-lookup"><span data-stu-id="d219f-117">Performs a logical OR operation on the color data for the eliminated pixels and the color data for the remaining pixels.</span></span>  |
| <span data-ttu-id="d219f-118">COLORONCOLOR</span><span class="sxs-lookup"><span data-stu-id="d219f-118">COLORONCOLOR</span></span> | <span data-ttu-id="d219f-119">Elimina completamente los datos de color de los píxeles eliminados.</span><span class="sxs-lookup"><span data-stu-id="d219f-119">Eliminates the color data of the deleted pixels completely.</span></span>                                                               |
| <span data-ttu-id="d219f-120">MEDIO</span><span class="sxs-lookup"><span data-stu-id="d219f-120">HALFTONE</span></span>     | <span data-ttu-id="d219f-121">Aproxima los datos de color originales (origen) en el destino.</span><span class="sxs-lookup"><span data-stu-id="d219f-121">Approximates the original (source) color data in the destination.</span></span>                                                         |



 

<span data-ttu-id="d219f-122">Establezca el modo de ajuste mediante una llamada a la función [**SetStretchBltMode**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) .</span><span class="sxs-lookup"><span data-stu-id="d219f-122">You set the stretch mode by calling the [**SetStretchBltMode**](/windows/desktop/api/Wingdi/nf-wingdi-setstretchbltmode) function.</span></span>

<span data-ttu-id="d219f-123">El siguiente código de ejemplo se toma de una aplicación que muestra los cuatro modos de stretch disponibles con la función [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) .</span><span class="sxs-lookup"><span data-stu-id="d219f-123">The following example code is taken from an application that demonstrates all four of the stretch modes available with the [**StretchBlt**](/windows/desktop/api/Wingdi/nf-wingdi-stretchblt) function.</span></span>
