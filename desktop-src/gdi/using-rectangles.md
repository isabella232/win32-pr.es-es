---
description: En el ejemplo de esta sección se muestra cómo usar las funciones de rectángulo. Consta del procedimiento de ventana principal de una aplicación que permite al usuario desplace y ajuste el tamaño de un mapa de bits.
ms.assetid: e8861240-9345-43e6-820d-d237247d1bd7
title: Usar rectángulos (Windows GDI)
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104002932"
---
# <a name="using-rectangles-windows-gdi"></a><span data-ttu-id="978d5-104">Usar rectángulos (Windows GDI)</span><span class="sxs-lookup"><span data-stu-id="978d5-104">Using Rectangles (Windows GDI)</span></span>

<span data-ttu-id="978d5-105">En el ejemplo de esta sección se muestra cómo usar las funciones de rectángulo.</span><span class="sxs-lookup"><span data-stu-id="978d5-105">The example in this section illustrates how to use the rectangle functions.</span></span> <span data-ttu-id="978d5-106">Consta del procedimiento de ventana principal de una aplicación que permite al usuario desplace y ajuste el tamaño de un mapa de bits.</span><span class="sxs-lookup"><span data-stu-id="978d5-106">It consists of the main window procedure from an application that enables the user to move and size a bitmap.</span></span>

<span data-ttu-id="978d5-107">Cuando se inicia la aplicación, se dibuja un mapa de bits de 32 píxeles de 32 píxeles en la esquina superior izquierda de la pantalla.</span><span class="sxs-lookup"><span data-stu-id="978d5-107">When the application starts, it draws a 32-pixel by 32-pixel bitmap in the upper left corner of the screen.</span></span> <span data-ttu-id="978d5-108">El usuario puede mover el mapa de bits arrastrándolo.</span><span class="sxs-lookup"><span data-stu-id="978d5-108">The user can move the bitmap by dragging it.</span></span> <span data-ttu-id="978d5-109">Para ajustar el tamaño del mapa de bits, el usuario crea un rectángulo de destino arrastrando el mouse y, a continuación, arrastra el mapa de bits y lo coloca en el rectángulo de destino.</span><span class="sxs-lookup"><span data-stu-id="978d5-109">To size the bitmap, the user creates a target rectangle by dragging the mouse, then drags the bitmap and "drops" it on the target rectangle.</span></span> <span data-ttu-id="978d5-110">La aplicación responde copiando el mapa de bits en el rectángulo de destino.</span><span class="sxs-lookup"><span data-stu-id="978d5-110">The application responds by copying the bitmap into the target rectangle.</span></span>

<span data-ttu-id="978d5-111">En el ejemplo siguiente se proporciona el procedimiento de ventana que permite al usuario trasladar y ajustar el tamaño del mapa de bits.</span><span class="sxs-lookup"><span data-stu-id="978d5-111">The window procedure that allows the user to move and size the bitmap is given in the following example.</span></span>
