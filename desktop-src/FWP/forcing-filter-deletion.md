---
title: Forzar la eliminación de filtros
description: En el código de ejemplo siguiente se muestra cómo hacer que un usuario con el privilegio TakeOwnership pueda eliminar un filtro.
...
ms.locfileid: "103778078"
---
# <a name="forcing-filter-deletion"></a><span data-ttu-id="aa6e6-103">Forzar la eliminación de filtros</span><span class="sxs-lookup"><span data-stu-id="aa6e6-103">Forcing Filter Deletion</span></span>

<span data-ttu-id="aa6e6-104">En el código de ejemplo siguiente se muestra cómo hacer que un usuario con el privilegio TakeOwnership pueda eliminar un filtro.</span><span class="sxs-lookup"><span data-stu-id="aa6e6-104">The following sample code demonstrates how to make a user with the TakeOwnership privilege capable of deleting a filter.</span></span>

> [!Note]  
> <span data-ttu-id="aa6e6-105">Una aplicación de instalación podría usar esta característica durante una desinstalación para asegurarse de que todos los objetos se han eliminado correctamente.</span><span class="sxs-lookup"><span data-stu-id="aa6e6-105">A setup application might use this feature during an uninstall to ensure that all objects are successfully deleted.</span></span>
