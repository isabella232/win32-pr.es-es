---
title: Propiedad Visible (objeto PropertySheet)
description: Obtenga información sobre la propiedad Visible del objeto PropertySheet, que devuelve o establece si la ventana de hoja de propiedades de Microsoft Agent está visible (abierta).
ms.assetid: 8a58cd45-0391-4897-b3a2-d4f5d7297ea8
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 64ca87a3fb7270a4842328e04ad48e36604ebffc5628aeafe8b734f36329c12d
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/11/2021
ms.locfileid: "117882206"
---
# <a name="visible-property-propertysheet-object"></a>Propiedad Visible (objeto PropertySheet)

\[Microsoft Agent está en desuso a partir Windows 7 y puede no estar disponible en versiones posteriores de Windows.\]

<dl> <dt>

<span id="Description"></span><span id="description"></span><span id="DESCRIPTION"></span>**Descripción**
</dt> <dd>

Devuelve o establece si la ventana de hoja de propiedades de Microsoft Agent está visible (abierta).

</dd> <dt>

<span id="Syntax_"></span><span id="syntax_"></span><span id="SYNTAX_"></span>**Sintaxis** 
</dt> <dd>

*agent!". PropertySheet.Visible* *  \[  =  *boolean*\]



| Parte      | Descripción                                                                                                                                                   |
|-----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *boolean* | Expresión booleana que especifica si la ventana está visible. **True** La ventana está visible.<br/> **False** La ventana está oculta (cerrada).<br/> |



 

</dd> </dl>

## <a name="remarks"></a>Comentarios

El servidor muestra la ventana en función de la ubicación y el tamaño establecidos por el usuario.

> [!Note]  
> El usuario puede invalidar esta propiedad.

 

 

 





