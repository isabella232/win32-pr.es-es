...
---
# <a name="wm_nclbuttonup-message"></a>Mensaje de NCLBUTTONUP de WM \_

Se envía cuando el usuario suelta el botón primario del mouse mientras el cursor se encuentra dentro del área no cliente de una ventana. Este mensaje se envía a la ventana que contiene el cursor. Si una ventana ha capturado el mouse, este mensaje no se publica.
...
La función [**DefWindowProc**](/windows/desktop/api/winuser/nf-winuser-defwindowproca) comprueba el punto especificado para averiguar la ubicación del cursor y realiza la acción adecuada. Si es necesario, **DefWindowProc** envía el mensaje de [**WM \_ SYSCOMMAND**](/windows/desktop/menurc/wm-syscommand) a la ventana.
