...
Realiza una operación de alineación de ROI, como se describe en el documento [Mask R-CNN .](https://arxiv.org/abs/1703.06870) En resumen, la operación extrae los recortes del tensor de la imagen de entrada y los cambia de tamaño a un tamaño de salida común especificado por las dos últimas dimensiones de *OutputTensor* mediante el *valor interpolationMode especificado.*
...
Tensor que contiene los índices por lotes de los que se extraerán los ROI. Las dimensiones permitidas de `BatchIndicesTensor` son , , o `{ NumROIs }` `{ 1, NumROIs }` `{ 1, 1, NumROIs }` `{ 1, 1, 1, NumROIs }` . Cada valor es el índice de un lote de *InputTensor.* El comportamiento no está definido si los valores no están en el intervalo [0, BatchCount).
