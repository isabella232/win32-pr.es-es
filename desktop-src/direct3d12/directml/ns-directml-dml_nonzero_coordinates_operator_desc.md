...
El rango efectivo de un tensor viene determinado por *dimensioncount* de ese tensor, excepto las dimensiones iniciales del tamaño 1. Por ejemplo, un tensor con tamaños de tiene el rango 3 efectivo, al igual `{1,2,3,4}` que un tensor con tamaños de `{1,1,5,5,5}` . Un tensor con tamaños `{1,1,1,1}` tiene un rango efectivo de 0.
...
> [!NOTE]
> Los elementos restantes de este tensor más allá de OutputCount no están definidos una vez que se completa este operador. No debe basarse en los valores de estos elementos.
