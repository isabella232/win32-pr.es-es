...
Tensor opcional que contiene los datos de punto cero de filtro. Las dimensiones esperadas de *FilterZeroPointTensor* son si se requiere una cuantificación por tensor o si se requiere una `{ 1, 1, 1, 1 }` `{ 1, OutputChannelCount, 1, 1 }` cuantificación por canal.
