...
---
# <a name="directml-helper-functions"></a>Funciones auxiliares de DirectML

## <a name="dmlcalcbuffertensorsize"></a>DMLCalcBufferTensorSize

Esta función auxiliar calcula el número mínimo de bytes necesarios para almacenar un búfer tensores con el tipo, los tamaños y los progresos especificados. La fórmula se puede expresar de la siguiente manera.
...
En otras palabras, el tamaño mínimo de un tensores es el índice del elemento que se encuentra una vez al final, multiplicado por el tamaño del elemento (por ejemplo, 2 bytes para un tensores de **FLOAT16** ). Además, DirectML requiere que todos los búferes enlazados tengan un tamaño total alineado con **DWORD** y, por lo tanto, el tamaño mínimo implícito en bytes se debe redondear al límite de 4 bytes más cercano.
...
Esta función auxiliar calcula los progresos para los decenas de 4D con el diseño NCHW o NHWC y la difusión opcional.
