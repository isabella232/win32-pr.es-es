...
description: La firma raíz es el patrimonio real principal y hay límites y costos estrictos que se deben tener en cuenta.
...
ms.locfileid: "118806665"
---
# <a name="root-signature-limits"></a>Límites de la firma raíz

La firma raíz es el patrimonio real principal y hay límites y costos estrictos que se deben tener en cuenta.
...
Este tamaño máximo se elige para evitar el abuso de la firma raíz como una manera de almacenar datos masivos. Cada entrada de la firma raíz tiene un costo para este límite de 64 DWORD:
...
El costo de rendimiento (en términos de niveles de direccionamiento indirecto) es cero para una constante raíz, 1 para un descriptor raíz y 2 para una tabla descriptor. Si una firma raíz es grande y se desborda de la memoria más rápida a una memoria ligeramente más lenta (lo que puede ocurrir en algún hardware), agregue 1 al costo de rendimiento de los elementos desbordados al final de la firma raíz.

Puede producirse un desbordamiento en hardware que podría tener, por ejemplo, un tamaño fijo de 16 DWORD para el espacio de argumentos raíz. Este límite podría reducirse aún más en uno si se usa el ensamblador de entrada. En este caso, hay desbordamiento en la memoria ligeramente más lenta si la firma raíz es demasiado grande para la memoria nativa de 15 o 16 DWORD. En otro hardware no hay memoria de argumento raíz nativa fija (por lo que la situación de desbordamiento nunca se produce).

Para todo el hardware, si cambia algún argumento raíz, el controlador debe mantener una versión de todos los argumentos raíz (a diferencia de otros almacenamientos, como los montones de descriptores y los recursos de búfer, que el controlador no tiene versiones). En el hardware en el que se produce una situación de desbordamiento, solo es necesario crear versiones del área nativa o de desbordamiento, en función de dónde se produjo el cambio. Obviamente, la cantidad de control de versiones debe mantenerse al mínimo necesario.
...
-   Organice los parámetros en una firma raíz grande para que los parámetros con mayor probabilidad cambien con frecuencia, o si la latencia de acceso baja para un parámetro determinado es importante, se produzca primero.
...
Los muestreadores estáticos (muestreadores en los que el estado está totalmente definido e inmutable) forman parte de las firmas raíz, pero no cuentan para el límite de 64 DWORD. Si un sampler se puede definir como estático, no es necesario que el muestreador forme parte de un montón de descriptores.
