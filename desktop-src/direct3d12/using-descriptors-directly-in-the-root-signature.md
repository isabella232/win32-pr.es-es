...
En el ejemplo anterior, no se puede declarar como una matriz, como en si se va a asignar a un descriptor en la `mySceneData` `cbuffer mySceneData[2]` firma raíz. Esto se debe a que la indexación entre descriptores no se admite en la firma raíz. Si lo desea, puede definir búferes de constantes individuales independientes y definirlos cada uno como una entrada independiente en la firma raíz. Tenga en cuenta `mySceneData` que, dentro de lo anterior, hay una matriz `bar[2]` . La indexación dinámica dentro del búfer constante es válida; un descriptor de la firma raíz se comporta igual que el mismo descriptor se comportaría si se accedía a él a través de un &mdash; montón de descriptores. Esto contrasta con la inlineación de constantes directamente en la firma raíz, que también aparece como un búfer constante, excepto con la restricción de que no se permite la indexación dinámica dentro de las constantes en línea, por lo que no se permitirá `bar[2]` allí.
