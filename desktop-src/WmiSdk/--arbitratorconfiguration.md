...
---
# <a name="__arbitratorconfiguration-class"></a>\_\_Clase ArbitratorConfiguration

La clase **\_ \_ ArbitratorConfiguration** es una clase de configuración que limita los recursos internos que usan las operaciones iniciadas por los clientes WMI. Se trata de una clase singleton que reside en el \\ espacio de nombres raíz. La clase se genera internamente, por lo que no hay ningún archivo MOF para ella.
...
La clase **\_ \_ ArbitratorConfiguration** tiene estos tipos de miembros:

-   [Propiedades](#properties)

### <a name="properties"></a>Propiedades

La clase **\_ \_ ArbitratorConfiguration** tiene estas propiedades.
...
Sin usar. Número de tareas iniciadas por el usuario pendientes al mismo tiempo.
...
Número de suscripciones permanentes permitidas para un usuario determinado al mismo tiempo.
...
Número total de suscripciones permanentes permitidas para todos los usuarios al mismo tiempo.
...
Número de consultas de eventos de sondeo permitidas para un usuario determinado al mismo tiempo.
...
Número total de instrucciones de sondeo permitidas para todos los usuarios al mismo tiempo.
...
La cantidad de consultas de eventos de sondeo de memoria, emitidas por un usuario determinado, puede consumir al mismo tiempo.
...
La cantidad total de memoria que sondea las consultas de eventos para todos los usuarios combinados puede realizarse en cualquier momento.
...
Sin usar. Número máximo de subprocesos de tarea asociados a un usuario determinado en cualquier momento.
...
Número de suscripciones temporales permitidas para un usuario determinado al mismo tiempo.
...
Número total de suscripciones temporales permitidas para todos los usuarios al mismo tiempo.
...
Sin usar. Total de caché de disco asociada a todos los usuarios al mismo tiempo.
...
Sin usar. Memoria caché de disco total asociada a una tarea determinada en un momento dado.
...
Sin usar. Total de memoria caché de disco asociada a un usuario determinado al mismo tiempo.
...
Sin usar. Memoria caché total asociada a todos los usuarios al mismo tiempo.
...
Sin usar. Memoria caché total asociada a una tarea determinada en un momento dado.
...
**\_ \_ ArbitratorConfiguration** se hereda de [**\_ \_ SystemClass**](--systemclass.md).
