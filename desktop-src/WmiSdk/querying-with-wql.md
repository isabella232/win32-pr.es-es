...
Para obtener una lista completa de las palabras clave WQL admitidas, vea [WQL (SQL para WMI).](wql-sql-for-wmi.md) El SQL palabras clave para los nombres de objeto o propiedad puede restringir el análisis de una consulta. Las siguientes SQL clave están restringidas: **NULL,** **TRUE** y **FALSE.**
...
Al igual que con SQL cadenas de consulta, puede escapar las consultas.

> [!Note]  
> WQL no admite consultas o asociaciones entre espacios de nombres. No se pueden consultar todas las instancias de una clase especificada que residan en todos los espacios de nombres del equipo de destino.
...
-   Consultas de eventos
    ...
    La siguiente consulta de evento de ejemplo realizada por un consumidor de eventos temporal solicita una notificación cuando se crea una nueva instancia de una clase derivada de [**Win32 \_ NTLogEvent.**](/previous-versions/windows/desktop/eventlogprov/win32-ntlogevent)
...
-   Consultas de esquema
    ...
    Las consultas de esquema se usan para recuperar definiciones de clase (en lugar de instancias de clase) y asociaciones de esquema. Los proveedores de clases usan consultas de esquema para especificar las clases que admiten cuando se registran. Para obtener más información sobre las consultas de esquema, vea [Recuperar definiciones de clase](retrieving-class-definitions.md).

    La siguiente consulta de esquema de ejemplo muestra la sintaxis especial.
