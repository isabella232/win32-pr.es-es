---
description: 'MUI: ejemplo de configuración de Application-Specific (anterior a Windows Vista)'
ms.assetid: 932aa981-ddd9-4a5b-9003-7dafd98e3ae4
title: 'MUI: ejemplo de configuración de Application-Specific (anterior a Windows Vista)'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103811299"
---
# <a name="mui-application-specific-settings-sample-pre-windows-vista"></a><span data-ttu-id="62890-103">MUI: ejemplo de configuración de Application-Specific (anterior a Windows Vista)</span><span class="sxs-lookup"><span data-stu-id="62890-103">MUI: Application-Specific Settings Sample (Pre-Windows Vista)</span></span>

<span data-ttu-id="62890-104">La aplicación de ejemplo que se describe en este tema es similar a la que se muestra en [MUI: Application-Specific Settings sample (Windows Vista)](mui-application-specific-settings-sample-vista.md), salvo que esta aplicación se ejecutará en Windows 2000 y versiones posteriores.</span><span class="sxs-lookup"><span data-stu-id="62890-104">The sample application described in this topic is similar to the one shown in [MUI: Application-Specific Settings Sample (Windows Vista)](mui-application-specific-settings-sample-vista.md), except that this application will run on Windows 2000 and later.</span></span> <span data-ttu-id="62890-105">La principal diferencia del otro ejemplo es que la reserva de idioma se debe personalizar para ejecutarse en varios sistemas operativos.</span><span class="sxs-lookup"><span data-stu-id="62890-105">The main difference from the other sample is that language fallback has to be customized to run on multiple operating systems.</span></span>

<span data-ttu-id="62890-106">Esta aplicación analiza primero una lista de idiomas delimitados en un archivo de texto y lo convierte en una lista de idiomas de cadena múltiple para definir las preferencias de idioma específico de la aplicación.</span><span class="sxs-lookup"><span data-stu-id="62890-106">This application first parses a delimited language list in a text file and converts it to a multi-string language list to define the application-specific language preferences.</span></span> <span data-ttu-id="62890-107">Los delimitadores admitidos en el ejemplo son ",", ";", ":" y "".</span><span class="sxs-lookup"><span data-stu-id="62890-107">The delimiters supported in the sample are ",", ";", ":", and " ".</span></span> <span data-ttu-id="62890-108">Después de analizar la lista, el código busca y carga los recursos en el idioma identificado.</span><span class="sxs-lookup"><span data-stu-id="62890-108">After parsing the list, the code finds and loads the resources in the identified language.</span></span> <span data-ttu-id="62890-109">A diferencia del ejemplo de otros valores de configuración específicos de la aplicación, este código carga y libera archivos de recursos mediante llamadas a las funciones de MUI [**LoadMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-loadmuilibrarya) y [**FreeMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-freemuilibrary).</span><span class="sxs-lookup"><span data-stu-id="62890-109">In contrast to the other application-specific settings sample, this code loads and releases resource files using calls to the MUI functions [**LoadMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-loadmuilibrarya) and [**FreeMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-freemuilibrary).</span></span>
