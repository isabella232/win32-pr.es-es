...
description: Lee muestras individuales de texturas multidimensionales de ejemplo 2.
...
ms.locfileid: "119043793"
---
# <a name="ld2dms-sm41---asm"></a>ld2dms (sm4.1 - asm)

Lee muestras individuales de texturas multidimensionales de ejemplo 2.



| ld2dms \[ \_ aoffimmi(u,v) \] dest \[ .mask \] , srcAddress \[ .swzzle, \] srcResource \[ .swzzle, \] sampleIndex (operando escalar) |
|------------------------------------------------------------------------------------------------------------------------|
...
| <span id="srcResource"></span><span id="srcresource"></span><span id="SRCRESOURCE"></span>*srcResource*<br/> | \[en Un registro de textura (t ) que se debe haber declarado para identificar qué textura o búfer se \] \# va a capturar.<br/> |
| <span id="sampleIndex"></span><span id="sampleindex"></span><span id="SAMPLEINDEX"></span>*sampleIndex*<br/> | \[en \] Idendifies los ejemplos que se leerán de *srcResource*.<br/>                                                       |
...
Esta instrucción es una alternativa simplificada a la [instrucción de](sample--sm4---asm-.md) ejemplo. Captura datos de la textura especificada sin ningún filtrado (por ejemplo, muestreo de punto) mediante el entero *srcAddress* y *sampleIndex proporcionados.*
...
*sampleIndex* no tiene que ser un literal. No es necesario especificar el recuento de varias muestras en el recurso de textura y funciona con vistas de profundidad o galería de símbolos.

Una aplicación que desee un control más flexible sobre  el comportamiento de direcciones fuera del intervalo debe usar la instrucción de ejemplo en su lugar, ya que respeta el comportamiento de encapsulado de direcciones, reflejo, fijación o borde definido como estado del muestreador.

*srcAddress.b* (post-swzzle) se omite para Texture2Ds. Si el valor está fuera del intervalo de índices de matriz disponibles \[ 0...( array size-1) , then \] **ld2dms** always returns 0 in all components present in the format of the resource, and defaults (0,0,0,1.0f/0x00000001) for missing components.

En el caso de las matrices Texture2D, *srcAddress.b* (posterior a swzzle) proporciona el índice de la matriz. Oherwise tiene el mismo comportamiento que Texture2D.

*srcAddress.a* (post-swzzle) siempre se omite. El compilador HLSL nunca mostrará nada allí.
...
El sufijo \[ \_ opcional aoffimmi(u,v,w) (desplazamiento de dirección por entero inmediato) indica que las coordenadas de textura de los \] **ld2dms** deben desplazarse por un conjunto de valores constantes enteros de espacio de textura inmediatos proporcionados. Los valores literales son un conjunto de números de complemento de 4 bits 2, con un intervalo entero \[ de -8,7 \] .
...
El resultado de especificar *un sampleIndex* que supere el número de muestras del recurso es indefinido, pero no puede devolver datos fuera del espacio de direcciones del contexto del dispositivo.
...
Al igual que con **la instrucción** de ejemplo, los valores devueltos para **ld2dms** son vectores de 4 con valores predeterminados específicos del formato para los componentes que no están presentes en el formato. El swzzle en *srcResource* determina cómo deslizar el resultado de 4 componentes procedente de la carga de textura, después de lo cual .mask en *dest* determina qué componentes *de dest* se actualizan.

Cuando **ld2dms** lee un valor float de 32 bits en un registro de 32 bits, los bits no se tocan; es decir, los valores desnormales permanecen desnormales. Esto es diferente de la **instrucción de** ejemplo.

### <a name="miscellaneous-details"></a>Detalles varios

Como no hay ningún filtrado asociado a esta instrucción, no se aplican conceptos como el sesgo de LOD. En consecuencia, no hay *ningún parámetro de sampler. \#*
