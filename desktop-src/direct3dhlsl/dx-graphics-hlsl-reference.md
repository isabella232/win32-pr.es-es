...
-   [**Funciones intrínsecas (HLSL de DirectX):**](dx-graphics-hlsl-intrinsic-functions.md) HLSL tiene muchas funciones intrínsecas. Se implementan y prueban para que pueda usarlas sabiendo que ya están depuradas y que tienen un buen rendimiento. Si decide escribir sus propias funciones, consulte la sección sintaxis del lenguaje para escribir funciones definidas por el usuario.
-   [Referencia de sombreador asm:](dx9-graphics-reference-asm.md) instrucciones de ensamblado que puede usar para programar y depurar sombreadores.
...
-   [Apéndice (HLSL de DirectX):](dx-graphics-hlsl-appendix.md) el apéndice se incluye para su integridad. Incluye una lista de las palabras clave y las palabras reservadas; estas palabras no se pueden usar como identificadores en los programas. También incluye una lista de la gramática del lenguaje como referencia.
