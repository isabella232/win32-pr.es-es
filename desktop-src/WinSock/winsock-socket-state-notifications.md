...
Si varios subprocesos están depurando paquetes de notificación en una conexión orientada a secuencias en la que los mensajes individuales deben procesarse en un único subproceso, considere la posibilidad de usar notificaciones de una sola toma desencadenadas por nivel. Esto reduce la probabilidad de que varios subprocesos reciban fragmentos de mensajes que deben volver a ensamblarse entre subprocesos. 

Si usa notificaciones desencadenadas por el perímetro, no se recomiendan las notificaciones de una sola toma porque el socket debe purgarse después de habilitar los registros. Se trata de un patrón más complicado de implementar, y es más costoso porque siempre requiere una llamada que devuelve **WSAEWOULDBLOCK**.

Si desea escalar horizontalmente la aceptación de la conexión en un único socket de escucha, los servidores deben usar la función [AcceptEx](/windows/win32/api/mswsock/nf-mswsock-acceptex) en lugar de suscribirse a las notificaciones de las solicitudes de conexión. La aceptación de conexiones en respuesta a las notificaciones limita implícitamente la tasa de aceptación de la conexión en relación con el procesamiento de solicitudes de conexiones existentes.

A continuación se muestran ejemplos de código que ilustran algunos escenarios de notificación de estado de socket. Parte del código contiene elementos *para hacer* para sus propias aplicaciones.

## <a name="common-code"></a>Código común

En primer lugar, esta es una lista de código que contiene algunas definiciones y funciones comunes que se usan en los escenarios siguientes.
...
## <a name="edge-triggered-udp-server"></a>Servidor UDP desencadenado por Edge

Esta es una ilustración sencilla de cómo usar las API con desencadenadores perimetrales.
