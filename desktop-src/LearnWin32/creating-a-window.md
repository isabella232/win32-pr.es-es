...
Puede leer descripciones detalladas de parámetros en MSDN, pero este es un resumen rápido:
...
- El último parámetro es un puntero a datos arbitrarios de tipo **void. \*** Puede usar este valor para pasar una estructura de datos al procedimiento de ventana. Mostraremos una posible manera de usar este parámetro en la sección [Administración del estado de la aplicación](managing-application-state-.md).
...
Enhorabuena, ha creado una ventana. En este momento, la ventana no contiene ningún contenido ni interactúa con el usuario. En una aplicación de GUI real, la ventana respondería a eventos del usuario y del sistema operativo. En la sección siguiente se describe cómo los mensajes de ventana proporcionan este tipo de interactividad.
