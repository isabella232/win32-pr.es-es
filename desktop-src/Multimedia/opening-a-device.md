...
Al abrir un dispositivo, puede usar la marca "tipo" para hacer referencia a un tipo de dispositivo, en lugar de a un controlador de dispositivo específico. En el ejemplo siguiente se abre el archivo de audio de forma de onda C: \\ WINDOWS \\ CHIMES. WAV (mediante la marca "type" para especificar el tipo de dispositivo **waveaudio)** y asigna el alias "chimes":
...
En la interfaz de mensaje de comando, el miembro **lpstrDeviceType** de la estructura [**\_ MCI OPEN \_ PARMS**](mci-open-parms.md) proporciona la funcionalidad de la marca "type".
...
-   Especifique un puntero a una cadena terminada en NULL que contenga el nombre del dispositivo del registro o del SYSTEM.INI predeterminado.
...
-   Especifique solo el nombre del dispositivo. Esto le permite abrir un dispositivo compuesto sin asociar un nombre de archivo. La mayoría de [](capability.md) los dispositivos compuestos solo procesan los comandos de funcionalidad [**\_ (MCI GETDEVCAPS)**](mci-getdevcaps.md)y [**cierran**](close.md) [**(MCI \_ CLOSE)**](mci-close.md)cuando se abren de esta manera.
...
En la interfaz de cadena de comandos, también puede abreviar la especificación del nombre del dispositivo mediante el formato de signo de exclamación alternativo, como se documenta con el [**comando**](open.md) open.

## <a name="opening-a-device-using-the-filename-extension"></a>Apertura de un dispositivo mediante la extensión filename

Si el [**comando open**](open.md) ([**MCI \_ OPEN**](mci-open.md)) especifica solo el nombre de archivo, MCI usa la extensión de nombre de archivo para seleccionar el dispositivo adecuado en la lista del Registro o en la sección de extensiones mci del archivo \[ \] SYSTEM.INI. Las entradas de la sección \[ de extensiones mci \] usan el formato siguiente:

*nombre de dispositivo \_ de extensión de* nombre de  =  *\_ archivo*
...
En el ejemplo siguiente se muestra una sección \[ de extensiones mci \] típicas:
...
Para crear un nuevo archivo de datos, basta con especificar un nombre de archivo en blanco. MCI no guarda un archivo nuevo hasta que lo guarde mediante el comando [**save**](save.md) [**(MCI \_ SAVE).**](mci-save.md) Al crear un nuevo archivo, debe incluir un alias de dispositivo con el [**comando open**](open.md) [**(MCI \_ OPEN).**](mci-open.md)

En el ejemplo siguiente se abre un nuevo **archivo waveaudio,** se inicia y se detiene la grabación, se guarda y se cierra el archivo:
