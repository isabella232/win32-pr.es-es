---
title: DrawDib
description: DrawDib
ms.assetid: c5e7237d-3a52-45b0-b6c5-13a1a8c1d50d
keywords:
- Windows multimedia, DrawDib
- multimedia, DrawDib
- Vídeo para Windows (VFW), DrawDib
- VFW (vídeo para Windows), DrawDib
- DrawDib, acerca de
- DrawDib, memoria de vídeo
- mapa de bits independiente de dispositivo (DIB)
- DIB (mapa de bits independiente del dispositivo)
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 63cfe88ec93e539fec74539c09d2f8a8446984ac
ms.sourcegitcommit: 2d531328b6ed82d4ad971a45a5131b430c5866f7
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 09/16/2019
ms.locfileid: "103994187"
---
# <a name="drawdib"></a>DrawDib

Las funciones DrawDib proporcionan funcionalidades de dibujo de imagen de alto rendimiento para mapas de bits independientes del dispositivo (DIB). Las funciones DrawDib admiten DIB de profundidades de imagen de 8 bits, de 16 bits, de 24 bits y 32 bits.

Las funciones DrawDib escriben directamente en la memoria de vídeo. No dependen de las funciones de la interfaz de dispositivo gráfico (GDI).

-   [Acerca de las funciones de DrawDib](about-the-drawdib-functions.md)
-   [Usar DrawDib](using-drawdib.md)
-   [Referencia de DrawDib](drawdib-reference.md)

 

 




