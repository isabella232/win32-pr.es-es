...
La información de esta sección solo se aplica a Microsoft Active Accessibility. No se aplica a las aplicaciones que usan Microsoft Automatización de la interfaz de usuario o las basadas en lenguajes de marcado como HTML, HTML dinámico (DHTML) o XML.

-   [Información general](#overview)
-   [Cómo provoca problemas la nomenclatura incorrecta](#how-incorrect-naming-causes-problems)
...
-   [Cómo dar nombre correctamente a una barra de seguimiento](#how-to-correctly-name-a-trackbar)
...
Microsoft Active Accessibility la cadena [de propiedad Name](name-property.md) de diferentes ubicaciones según el tipo del elemento de interfaz de usuario. Para la mayoría de los elementos de la interfaz de usuario que tienen texto de ventana asociado, Microsoft Active Accessibility el texto de la ventana como la cadena de propiedad Name. Entre los ejemplos de este tipo de elemento de interfaz de usuario se incluyen controles como botones, elementos de menú e información sobre herramientas.

Para los controles siguientes, Microsoft Active Accessibility el texto de la ventana y, en su lugar, busca una etiqueta de texto estático (o etiqueta de cuadro de grupo) inmediatamente anterior al control en el orden de tabulación.
...
La mayoría de los controles anteriores tienen realmente texto de ventana asociado. El editor de recursos genera automáticamente el texto de la ventana, que consta de una cadena genérica como "edit1" o "listbox3". Aunque los desarrolladores pueden reemplazar el texto de ventana generado por texto más significativo, la mayoría nunca lo hace. Dado que el texto de la ventana generada no tiene ningún significado para el usuario, Microsoft Active Accessibility omite y usa en su lugar la etiqueta de texto estático que lo acompaña.
...
En la captura de pantalla anterior, observe que la jerarquía de objetos no coincide con la estructura de los controles tal como aparecen en la interfaz de usuario del formulario de entrada de nombre. Observe también que [Inspeccionar](inspect-objects.md) ha asignado el nombre incorrecto al elemento siguiente al último elemento (es el control de edición para escribir el nombre y debe tener el nombre "First Name:". Por último, observe que Inspect no pudo encontrar un nombre para el último elemento (es el control de edición para escribir el apellido y debe tener un nombre de "Apellidos:".

En el ejemplo siguiente se muestra el contenido del archivo de recursos para el formulario de entrada de nombre. Observe que el orden de tabulación no es coherente con la estructura lógica de los controles tal como aparecen en la interfaz de usuario. Observe también que no se especifica ninguna tecla de método abreviado para los dos controles de edición.
...
Para realizar correcciones en un archivo de recursos, puede editar el archivo directamente o puede usar la herramienta Orden de tabulación en Microsoft Visual Studio. Para acceder a la herramienta Orden de tabulación en Visual Studio presione CTRL+D o seleccione Orden de tabulación **en** el **menú** Formato.
...
## <a name="how-to-correctly-name-a-trackbar"></a>Cómo dar nombre correctamente a una barra de seguimiento
...
Al igual que con las etiquetas visibles, una etiqueta invisible debe preceder inmediatamente al control en el orden de tabulación. Para que una etiqueta sea invisible en un archivo de recursos (.rc), agregue o a la parte `NOT WS_VISIBLE` de estilo del control de texto `|~WS_VISIBLE` estático. Si usa el Editor de recursos en Visual Studio, puede establecer la propiedad Visible en False.
...
2.  Inicialice la biblioteca Del modelo de objetos componentes (COM) llamando a la [función CoInitializeEx,](/windows/win32/api/combaseapi/nf-combaseapi-coinitializeex) normalmente durante el proceso de inicialización de la aplicación.
