...
> [!Important]
> **Inspeccionar** es una herramienta heredada. En su lugar, [se recomienda usar Accessibility Insights.](https://accessibilityinsights.io/)

**Inspeccionar** (Inspect.exe) es una herramienta basada en Windows que permite seleccionar cualquier elemento de la interfaz de usuario y ver los datos de accesibilidad del elemento. Puedes ver las propiedades de Automatización de la interfaz de usuario de Microsoft y patrones de control, así como las propiedades de Microsoft Active Accessibility. **Inspeccionar** también permite probar la estructura de navegación de los elementos de automatización del árbol de Automatización de la interfaz de usuario y los objetos accesibles en la Microsoft Active Accessibility de navegación.
...
**Inspect** se instala como parte del conjunto general de herramientas de Kit de desarrollo de software de Windows (SDK), no se distribuye como una descarga independiente. El Windows SDK incluye todas las herramientas relacionadas con la accesibilidad que se documentan en esta sección.

[Descargue el Windows SDK](https://developer.microsoft.com/en-us/windows/downloads/windows-10-sdk/).

> [!NOTE]
> Para las versiones anteriores de la Windows SDK, vea el archivo [Windows SDK y el emulador](https://developer.microsoft.com/en-us/windows/downloads/sdk-archive/).

**Inspect.exe** se encuentra en la carpeta bin version platform> de la ruta de instalación del SDK (normalmente no tiene que ejecutar \\ \\ <  > \\ <  como administrador).
...
### <a name="configure-property-settings"></a>Configuración de valores de propiedad
...
| Vista de solo Windows visible    | Disponible solo en modo MSAA.                                                                                                                                                                                                       |
...
| Vista de control                 | Presenta la [vista de](uiauto-treeoverview.md) control del Automatización de la interfaz de usuario árbol en la **vista** Árbol. Solo está disponible Automatización de la interfaz de usuario modo de configuración.                                                                            |
...
| Bip on Error                | Pitidos cuando se detecta un error durante una operación Automatización de la interfaz de usuario o MSAA.                                                                                                                                                          |
...
| Foco de reloj                  | Sigue el foco del teclado. Cuando se selecciona, se instala un enlace de eventos de foco asincrónico y mueve el elemento de subrayado a la parte superior izquierda del elemento con el foco. Esto hace **que Inspect** actualice sus propiedades en aproximadamente un segundo. |
| Watch Caret                  | Sigue el caret. Disponible solo en modo MSAA.                                                                                                                                                                                    |
| Cursor de reloj                 | Sigue al cursor.                                                                                                                                                                                                                |
| Visualización de información sobre herramientas               | Sigue la información sobre herramientas.                                                                                                                                                                                                              |
...
Una vez que haya seleccionado un elemento de interfaz de usuario mediante **Inspeccionar**, puede validar que el elemento expone la navegación correcta de Automatización de Windows para productos de tecnología de asistencia.
...
3. En la **vista** Datos, compruebe que el elemento expone las propiedades relacionadas con la navegación correctas.
4. Use la **vista**  Árbol, el menú Navegación o los botones de navegación de la barra de herramientas para navegar por la interfaz de usuario y comprobar que cada elemento expone las propiedades relacionadas con la navegación correctas.
    > [!Note]  
...
Windows Automation expone métodos que permiten a los productos de tecnología de asistencia interactuar con un elemento de interfaz de usuario como si se usara el mouse o el teclado (por ejemplo, para hacer clic en un botón). El **menú Inspeccionar** acción permite a los evaluadores invocar métodos de Automatización de Windows en un elemento (por ejemplo, **Invoke.Invoke** llama al método [**IUIAutomationInvokePattern::Invoke).**](/windows/desktop/api/UIAutomationClient/nf-uiautomationclient-iuiautomationinvokepattern-invoke)
...
3. En el **menú Acción** o la barra de herramientas, seleccione la acción correspondiente al método de Automatización de Windows que desea invocar.
...
| Elemento secundario centrado         | Navega al elemento secundario del elemento que tiene actualmente el foco.                                                       |
| HitTest en el cursor     | Navega hasta el elemento secundario del elemento especificado por el cursor del mouse.                                                      |
...
| Establezca el foco del teclado en el objeto situado bajo el cursor (Foco).                                                                                   | CTRL+MAYÚS+F4              |
| Mueva al objeto relacionado anterior desde el que está debajo del cursor. Este comando navega a objetos solo dentro de un contenedor (anterior del mismo nivel). | CTRL+MAYÚS+F5              |
| Ir al elemento primario del objeto (primario).                                                                                                            | CTRL+MAYÚS+F6              |
...
| Desplazarse al último elemento secundario del objeto actual (Último elemento secundario).                                                                                       | CTRL+MAYÚS+F9              |
...
| Observe el objeto que tiene el foco (Watch Focus).                                                                                                   | CTRL+MAYÚS+6               |
| Moverse al objeto relacionado a la izquierda del que está sobre el cursor (Izquierda). Disponible solo en modo MSAA.                                        | CTRL+MAYÚS+7               |
| Mueva al objeto relacionado situado encima del objeto sobre el que está el cursor (Arriba). Disponible solo en modo MSAA.                                                | CTRL+MAYÚS+8               |
| Mueva al objeto relacionado debajo del que está sobre el que está el cursor (Abajo). Disponible solo en modo MSAA.                                                 | CTRL+MAYÚS+9               |
| Desplazarse al objeto relacionado a la derecha del que está sobre el cursor (derecha). Disponible solo en modo MSAA.                                      | CTRL+MAYÚS+0               |
