...
description: En este tema se proporciona información sobre microsoft Automatización de la interfaz de usuario compatibilidad con el tipo de control Hyperlink.
...
|---------------------------------------------------|------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**IInvokeProvider**](/windows/desktop/api/UIAutomationCore/nn-uiautomationcore-iinvokeprovider) | Requerido                     | Todos los controles de hipervínculo deben admitir el patrón de control [Invoke.](uiauto-implementinginvoke.md)                                                                                                                                                       |
...
| [**Valor**](/windows/desktop/api/UIAutomationCore/nf-uiautomationcore-ivalueprovider-get_value)      | Por ejemplo, "https://www..". | Una dirección URL para una dirección de Internet o intranet es un ejemplo de un hipervínculo que contiene información significativa para el usuario. Sin embargo, un vínculo mediante programación solo es significativo para una aplicación y no se recomienda para la **propiedad Value.** |
...
El tipo de control Hyperlink solo se debe aplicar a un objeto que, al hacer clic en él, hace que se produzca la navegación; no se debe aplicar al contenedor del hipervínculo. Por ejemplo, solo las "zonas de acceso más populares" en las que se puede hacer clic dentro de un mapa de imágenes deben tener el tipo de control **Hyperlink.** Lo mismo sucede con los hipervínculos de un campo de texto o contenedor de documentos. En este caso, solo el texto o la imagen del hipervínculo debe tener el tipo de control **Hyperlink,** no el contenedor.

El [patrón de](uiauto-implementingtextandtextrange.md) control Texto es ideal para admitir hipervínculos incrustados en elementos de texto o documento.
