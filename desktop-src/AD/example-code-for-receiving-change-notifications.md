---
title: Código de ejemplo para recibir notificaciones de cambios
description: En el ejemplo de código siguiente se muestra cómo usar el control de notificación de cambios de LDAP para recibir notificaciones de cambios en un objeto de Active Directory Domain Services.
ms.assetid: e1d36d1c-ee50-4c2f-92f6-eee1dae1c5af
ms.tgt_platform: multiple
keywords:
- Active Directory ejemplos Active Directory, recibir notificaciones de cambios
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103995089"
---
# <a name="example-code-for-receiving-change-notifications"></a><span data-ttu-id="e05fc-104">Código de ejemplo para recibir notificaciones de cambios</span><span class="sxs-lookup"><span data-stu-id="e05fc-104">Example Code for Receiving Change Notifications</span></span>

<span data-ttu-id="e05fc-105">En el ejemplo de código siguiente se muestra cómo usar el control de notificación de cambios de LDAP para recibir notificaciones de cambios en un objeto de Active Directory Domain Services.</span><span class="sxs-lookup"><span data-stu-id="e05fc-105">The following code example shows how to use the LDAP change notification control to receive notifications of changes to an object in Active Directory Domain Services.</span></span> <span data-ttu-id="e05fc-106">El ejemplo se registra para las notificaciones, lee el estado inicial del objeto y, a continuación, usa un bucle para esperar y procesar los cambios en el objeto.</span><span class="sxs-lookup"><span data-stu-id="e05fc-106">The example registers for notifications, reads the initial state of the object, and then uses a loop to wait for and process changes to the object.</span></span>

<span data-ttu-id="e05fc-107">En primer lugar, el ejemplo de código llama a la función [**\_ \_ ext de búsqueda LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_ext) , que es una operación de búsqueda asincrónica que devuelve después de registrar una solicitud de notificación.</span><span class="sxs-lookup"><span data-stu-id="e05fc-107">First, the code example calls the [**ldap\_search\_ext**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_ext) function, which is an asynchronous search operation that returns after registering a notification request.</span></span> <span data-ttu-id="e05fc-108">En segundo lugar, después de configurar la solicitud de notificación, el ejemplo llama a la función de [**búsqueda LDAP, que es una \_ \_**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_s) operación de búsqueda sincrónica que lee el estado actual del objeto.</span><span class="sxs-lookup"><span data-stu-id="e05fc-108">Second, after setting up the notification request, the example calls the [**ldap\_search\_s**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_s) function, which is a synchronous search operation that reads the current state of the object.</span></span> <span data-ttu-id="e05fc-109">En tercer lugar, el ejemplo utiliza un bucle que llama al \_ resultado de LDAP para esperar los resultados de la operación de búsqueda asincrónica.</span><span class="sxs-lookup"><span data-stu-id="e05fc-109">Third, the example, uses a loop that calls ldap\_result to wait for results of the asynchronous search operation.</span></span> <span data-ttu-id="e05fc-110">Cuando se devuelve la función de [**\_ resultado LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) , en el ejemplo se procesan los resultados de la búsqueda y se repite el bucle.</span><span class="sxs-lookup"><span data-stu-id="e05fc-110">When the [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) function returns, the example processes the search results and repeats the loop.</span></span>

<span data-ttu-id="e05fc-111">Tenga en cuenta que si en el ejemplo se lee el estado actual y, a continuación, se configura la solicitud de notificación, habrá un período de tiempo durante el cual podrían producirse cambios antes de que se registrara la solicitud de notificación.</span><span class="sxs-lookup"><span data-stu-id="e05fc-111">Be aware that if the example read the current state and then set up the notification request, there would be a period of time during which changes could occur before the notification request was registered.</span></span> <span data-ttu-id="e05fc-112">Al leer el objeto después de configurar la solicitud de notificación, la ventana funciona en orden inverso: podría recibir notificaciones de los cambios que se produjeron antes de leer el estado inicial.</span><span class="sxs-lookup"><span data-stu-id="e05fc-112">By reading the object after setting up the notification request, the window works in reverse–you could receive notifications of changes that occurred before reading the initial state.</span></span> <span data-ttu-id="e05fc-113">Para controlar esto, en el ejemplo se almacena en caché el valor del atributo [**uSNChanged**](/windows/desktop/ADSchema/a-usnchanged) del objeto cuando lee el estado inicial del objeto.</span><span class="sxs-lookup"><span data-stu-id="e05fc-113">To handle this, the example caches the object [**uSNChanged**](/windows/desktop/ADSchema/a-usnchanged) attribute value when it reads the object's initial state.</span></span> <span data-ttu-id="e05fc-114">A continuación, cuando el [**\_ resultado LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) se devuelve con una notificación de cambio, en el ejemplo se compara el valor del atributo **uSNChanged** almacenado en caché con el valor notificado por el **\_ resultado LDAP**.</span><span class="sxs-lookup"><span data-stu-id="e05fc-114">Then, when [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) returns with a change notification, the example compares the cached **uSNChanged** attribute value with the value reported by **ldap\_result**.</span></span> <span data-ttu-id="e05fc-115">Si el nuevo valor del atributo **uSNChanged** es menor o igual que el valor almacenado en caché, el ejemplo descarta los resultados porque indican un cambio que se produjo antes de la operación de lectura inicial.</span><span class="sxs-lookup"><span data-stu-id="e05fc-115">If the new **uSNChanged** attribute value is less than or equal to the cached value, the example discards the results because they indicate a change that occurred prior to the initial read operation.</span></span>

<span data-ttu-id="e05fc-116">En este ejemplo se realiza una búsqueda de nivel base que supervisa un solo objeto.</span><span class="sxs-lookup"><span data-stu-id="e05fc-116">This example performs a base level search that monitors a single object.</span></span> <span data-ttu-id="e05fc-117">Puede especificar el ámbito **de \_ ámbito \_ de LDAP ONELEVEL** para supervisar todos los objetos secundarios del objeto especificado.</span><span class="sxs-lookup"><span data-stu-id="e05fc-117">You can specify the **LDAP\_SCOPE\_ONELEVEL** scope to monitor all child objects of the specified object.</span></span> <span data-ttu-id="e05fc-118">También puede modificar el código para registrar hasta cinco solicitudes de notificación y, a continuación, usar la función de [**\_ resultado LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) para esperar las notificaciones de cualquiera de las solicitudes.</span><span class="sxs-lookup"><span data-stu-id="e05fc-118">You can also modify the code to register up to five notification requests and then use the [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) function to wait for notifications from any of the requests.</span></span> <span data-ttu-id="e05fc-119">Recuerde que las solicitudes de notificación de cambios afectan al rendimiento del servidor.</span><span class="sxs-lookup"><span data-stu-id="e05fc-119">Remember that change notification requests impact the performance of the server.</span></span> <span data-ttu-id="e05fc-120">Para obtener más información sobre cómo mejorar el rendimiento, consulte [notificaciones de cambio en Active Directory Domain Services](change-notifications-in-active-directory-domain-services.md).</span><span class="sxs-lookup"><span data-stu-id="e05fc-120">For more information about how to improve performance, see [Change Notifications in Active Directory Domain Services](change-notifications-in-active-directory-domain-services.md).</span></span>
