---
title: Leer el descriptor de seguridad de un objeto
description: En el ejemplo de código siguiente se utilizan las interfaces de IADs para enumerar las propiedades del descriptor de seguridad de un objeto de directorio, la DACL y las ACE de la DACL.
ms.assetid: a8d0a6aa-9fbd-4392-b28b-f2eadef3935c
ms.tgt_platform: multiple
keywords:
- Leer el anuncio del descriptor de seguridad de un objeto
- Active Directory, ejemplo de código Visual Basic, leer el descriptor de seguridad de un objeto
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103904474"
---
# <a name="reading-an-objects-security-descriptor"></a><span data-ttu-id="a8d9e-105">Leer el descriptor de seguridad de un objeto</span><span class="sxs-lookup"><span data-stu-id="a8d9e-105">Reading an Object's Security Descriptor</span></span>

<span data-ttu-id="a8d9e-106">En el ejemplo de código siguiente se utilizan las interfaces de [**IADs**](/windows/desktop/api/iads/nn-iads-iads) para enumerar las propiedades del descriptor de seguridad de un objeto de directorio, la DACL y las ACE de la DACL.</span><span class="sxs-lookup"><span data-stu-id="a8d9e-106">The following code example uses the [**IADs**](/windows/desktop/api/iads/nn-iads-iads) interfaces to enumerate the properties of a directory object's security descriptor, DACL, and the ACEs of the DACL.</span></span>

<span data-ttu-id="a8d9e-107">En el ejemplo de código siguiente se usa el método [**IADs. Get**](/windows/desktop/api/iads/nf-iads-iads-get) para recuperar la propiedad **nTSecurityDescriptor** del objeto de directorio.</span><span class="sxs-lookup"><span data-stu-id="a8d9e-107">The following code example uses the [**IADs.Get**](/windows/desktop/api/iads/nf-iads-iads-get) method to retrieve the **nTSecurityDescriptor** property of the directory object.</span></span> <span data-ttu-id="a8d9e-108">La versión de C++ de este método devuelve un **valor de tipo Variant** que contiene un puntero [**IDispatch**](/windows/win32/api/oaidl/nn-oaidl-idispatch) .</span><span class="sxs-lookup"><span data-stu-id="a8d9e-108">The C++ version of this method returns a **VARIANT** that contains an [**IDispatch**](/windows/win32/api/oaidl/nn-oaidl-idispatch) pointer.</span></span> <span data-ttu-id="a8d9e-109">A continuación, en el ejemplo de código se llama a **QueryInterface** en ese puntero **IDispatch** para obtener una interfaz [**IADsSecurityDescriptor**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) en el descriptor de seguridad del objeto.</span><span class="sxs-lookup"><span data-stu-id="a8d9e-109">The code example then calls **QueryInterface** on that **IDispatch** pointer to get an [**IADsSecurityDescriptor**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) interface to the object's security descriptor.</span></span>

<span data-ttu-id="a8d9e-110">A continuación, en el ejemplo de código se usan métodos [**IADsSecurityDescriptor**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) para recuperar datos del descriptor de seguridad.</span><span class="sxs-lookup"><span data-stu-id="a8d9e-110">The code example then uses [**IADsSecurityDescriptor**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) methods to retrieve data from the security descriptor.</span></span> <span data-ttu-id="a8d9e-111">Tenga en cuenta que los datos, disponibles a través de **IADsSecurityDescriptor**, dependen de los derechos de acceso del autor de la llamada.</span><span class="sxs-lookup"><span data-stu-id="a8d9e-111">Be aware that the data, available through **IADsSecurityDescriptor**, depends on the access rights of the caller.</span></span> <span data-ttu-id="a8d9e-112">Se producirá un error en las propiedades [**IADsSecurityDescriptor. DiscretionaryAcl**](/windows/desktop/ADSI/iadssecuritydescriptor-property-methods) y **IADsSecurityDescriptor. Owner** si el autor de la llamada no tiene acceso de **\_ control de lectura** al objeto.</span><span class="sxs-lookup"><span data-stu-id="a8d9e-112">The [**IADsSecurityDescriptor.DiscretionaryAcl**](/windows/desktop/ADSI/iadssecuritydescriptor-property-methods) and **IADsSecurityDescriptor.Owner** properties will fail if the caller does not have **READ\_CONTROL** access to the object.</span></span> <span data-ttu-id="a8d9e-113">Del mismo modo, se produciría un error en una llamada al método **Get \_ SystemAcl** si el autor de la llamada no tiene habilitado el privilegio de **\_ \_ nombre de seguridad se** .</span><span class="sxs-lookup"><span data-stu-id="a8d9e-113">Similarly, a call to the **get\_SystemAcl** method would fail if the caller does not have the **SE\_SECURITY\_NAME** privilege enabled.</span></span>
