...
---
# <a name="reading-an-objects-security-descriptor"></a>Leer el descriptor de seguridad de un objeto

En el ejemplo de código siguiente se utilizan las interfaces de [**IADs**](/windows/desktop/api/iads/nn-iads-iads) para enumerar las propiedades del descriptor de seguridad de un objeto de directorio, la DACL y las ACE de la DACL.

En el ejemplo de código siguiente se usa el método [**IADs. Get**](/windows/desktop/api/iads/nf-iads-iads-get) para recuperar la propiedad **nTSecurityDescriptor** del objeto de directorio. La versión de C++ de este método devuelve un **valor de tipo Variant** que contiene un puntero [**IDispatch**](/windows/win32/api/oaidl/nn-oaidl-idispatch) . A continuación, en el ejemplo de código se llama a **QueryInterface** en ese puntero **IDispatch** para obtener una interfaz [**IADsSecurityDescriptor**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) en el descriptor de seguridad del objeto.

A continuación, en el ejemplo de código se usan métodos [**IADsSecurityDescriptor**](/windows/desktop/api/iads/nn-iads-iadssecuritydescriptor) para recuperar datos del descriptor de seguridad. Tenga en cuenta que los datos, disponibles a través de **IADsSecurityDescriptor**, dependen de los derechos de acceso del autor de la llamada. Se producirá un error en las propiedades [**IADsSecurityDescriptor. DiscretionaryAcl**](/windows/desktop/ADSI/iadssecuritydescriptor-property-methods) y **IADsSecurityDescriptor. Owner** si el autor de la llamada no tiene acceso de **\_ control de lectura** al objeto. Del mismo modo, se produciría un error en una llamada al método **Get \_ SystemAcl** si el autor de la llamada no tiene habilitado el privilegio de **\_ \_ nombre de seguridad se** .
