...
Cuando la aplicación de servidor COM+ se inicia manualmente, actúa como un host DLL con la configuración de seguridad de un servicio. El servicio se iniciará manualmente cuando se active y se apague automáticamente cuando se haya cerrado el tiempo de espera.
...
La aplicación también se puede iniciar en cualquier orden concreto seleccionando las dependencias de una lista de otros servicios del sistema. Por ejemplo, los servicios del sistema se pueden marcar como dependientes y no iniciarán la aplicación hasta que los servicios del sistema se hayan iniciado en el orden especificado. Esto inicializará correctamente la aplicación de servicio antes de que se utilice.

Para obtener instrucciones paso a paso sobre cómo configurar una aplicación COM+ para que se ejecute como un servicio, vea [Configuring a COM+ Server Application as a Service Application](configuring-a-com--server-application-as-a-service-application.md).
