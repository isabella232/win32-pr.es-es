...
Dado que las aplicaciones de biblioteca COM+ se hospedan en otros procesos, sus requisitos de seguridad pueden ser diferentes de los de las aplicaciones de servidor. Si la aplicación de biblioteca se hospedará en un explorador, es posible que tenga que recibir devoluciones de llamada no autenticadas.

Para solucionar esta necesidad, puede deshabilitar la autenticación para que el proceso de hospedaje no realice comprobaciones de seguridad para los llamadores de la aplicación de biblioteca. Al deshabilitar la autenticación, las llamadas a la aplicación de biblioteca no se autentican de forma eficaz, ya que todas las llamadas a ella se realizará correctamente. Para obtener más información, vea [Library Application Security](library-application-security.md).
