...
Para generar un evento, el publicador crea una instancia del objeto de clase de evento llamando a [**CoCreateInstance**](/windows/desktop/api/combaseapi/nf-combaseapi-cocreateinstance) o al método **CreateObject** de Microsoft Visual Basic y solicitando que se devuelva la interfaz de eventos. El objeto de clase de eventos con instancias contiene la implementación del sistema de eventos de la interfaz solicitada. Un suscriptor interesado también debe implementar la interfaz de clase de eventos para recibir eventos de un publicador determinado. Cuando se crea una instancia del objeto de clase de eventos, el sistema de eventos lo asocia a los suscriptores adecuados. La lista de suscriptores se mantiene durante la vigencia del objeto de clase de evento. Un evento se puede entregar a varios suscriptores en serie o en paralelo.
