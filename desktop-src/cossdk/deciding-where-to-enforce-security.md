...
-   Una amplia variedad de aplicaciones acceden a la base de datos y su seguridad no se puede configurar en función de una sola aplicación.
-   Una base de datos se puede proteger de forma predecible. Si mantiene datos críticos para la empresa en una base de datos, puede dibujar un vagón ajustado alrededor de él y ayudar a controlar quién puede verlos.
-   La autenticación de clientes originales en la base de datos permite el registro detallado de quién ha hecho lo que se ha hecho en la base de datos.
...
Cuando se tiene la dicha de diseñar la seguridad de la base de datos y la aplicación COM+ conjuntamente, la mayoría de estos problemas pertenecen a las características de los propios datos y su relación con los usuarios que acceden a ellos. Con los datos a los que se puede acceder mediante categorías de usuarios predecibles, es eficaz controlar el acceso en el nivel intermedio mediante roles. Con datos que están estrechamente enlazados a clases muy pequeñas de usuarios, esas relaciones a menudo se deben expresar con los propios datos y, por tanto, debe aplicar cierta seguridad en la base de datos.
...
Si autentica o autoriza a los usuarios en la base de datos, la identidad del usuario debe propagarse a la base de datos para admitirlo. Si la base de datos confía en la aplicación de nivel intermedio para hacerlo, es posible pasar información de seguridad de usuario en los parámetros. De lo contrario, la llamada a la base de datos se debe realizar en la identidad del usuario, lo que significa que la aplicación de servidor debe suplantar al cliente. Esto puede tener implicaciones de rendimiento como las siguientes:

-   La suplantación del cliente puede ser mucho más lenta que realizar la llamada directamente en la identidad de la aplicación. Para obtener más información, [vea Suplantación de cliente y Delegación.](client-impersonation-and-delegation.md)
