...
-   El registro Windows (**RAÍZ HKEY \_ CLASSES \_**)
...
En el caso de los componentes COM existentes que no requieren nuevos servicios COM+, la búsqueda se produce en el registro Windows existente. El catálogo de COM+ también usa el registro Windows para el registro de código auxiliar o proxy de interfaz y biblioteca de tipos.

## <a name="split-registration"></a>Dividir registro

En el caso de los nuevos componentes que ya son componentes COM existentes que se usan en el entorno de servicios (por ejemplo, los componentes mts), el aspecto COM básico del registro se almacena en el registro de Windows y los nuevos servicios y atributos (por ejemplo, los componentes en cola) se almacenan en la base de datos de registro de COM+. Esto se conoce como un *registro dividido.*

Cada atributo se almacena en una sola ubicación: el registro Windows o la base de datos de registro de COM+. Los nuevos componentes COM se registran exclusivamente en la base de datos de registro de COM+, con alguna duplicación en el registro de Windows para que las herramientas existentes puedan usarlas.
...
Sin embargo, no se garantiza que las actualizaciones que implican cambios en otros almacenes (como el sistema de archivos y Windows registro) sean completamente transaccionales. Una transacción anulada puede dejar estos almacenes en un estado incoherente con los cambios realizados entre sí o en la base de datos de registro de COM+.
