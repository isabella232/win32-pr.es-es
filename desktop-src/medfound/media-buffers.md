...
Un búfer multimedia es un objeto COM que administra un bloque de memoria, normalmente para contener datos multimedia. Los búferes de medios se utilizan para trasladar datos de un componente de canalización al siguiente. La mayoría de las aplicaciones no usan búferes multimedia directamente, ya que la sesión multimedia controla todo el flujo de datos entre los objetos de canalización. Debe usar búferes multimedia si está escribiendo su propio componente de canalización o si usa un componente de canalización directamente sin la sesión multimedia.

Los búferes multimedia exponen la interfaz [**IMFMediaBuffer**](/windows/desktop/api/mfobjects/nn-mfobjects-imfmediabuffer) . Esta interfaz está diseñada para leer o escribir cualquier tipo de datos. Los fotogramas de vídeo sin comprimir requieren un tratamiento especial, ya que pueden almacenarse en superficies de Direct3D ubicadas en la memoria de vídeo.
