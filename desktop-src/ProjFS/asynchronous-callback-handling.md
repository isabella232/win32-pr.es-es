...
description: Describe cómo el proveedor puede realizar devoluciones de llamada de servicio de forma asincrónica.
...
Cuando un cliente interactúa con los archivos y directorios debajo de la raíz de virtualización del proveedor, estas interacciones suelen dar lugar a que se invoque la devolución de llamada del proveedor.  ProjFS invoca las devoluciones de llamada del proveedor mediante el envío de un mensaje desde el modo kernel a la biblioteca de modo de usuario ProjFS, donde un subproceso de trabajo recibe el mensaje y llama a la devolución de llamada adecuada.  Una vez devuelta la devolución de llamada, el subproceso de trabajo espera a que llegue otro mensaje desde el modo kernel.  Si todos los subprocesos de trabajo están ocupados ejecutando el código de devolución de llamada del proveedor, cualquier E/S de cliente adicional que descadene una devolución de llamada se bloqueará hasta que un subproceso de trabajo esté disponible para recibir el mensaje e invocar la devolución de llamada correspondiente.  Cuando se inicia un proveedor, puede especificar el número de subprocesos de trabajo que quiere que ProjFS cree para dar servicio a las devoluciones de llamada a través del parámetro _options_ **[de PrjStartVirtualizing](/windows/desktop/api/projectedfslib/nf-projectedfslib-prjstartvirtualizing)**.  Un proveedor puede mejorar la eficacia de los subprocesos de trabajo que reciben mensajes mediante el mantenimiento de sus devoluciones de llamada de forma asincrónica.

> Si el proveedor no especifica el parámetro _options_ en **PrjStartVirtualizing** o si especifica 0 para el miembro ConcurrentThreadCount del parámetro _options,_ ProjFS usará el número de procesadores lógicos del sistema para el valor de ConcurrentThreadCount.
>
