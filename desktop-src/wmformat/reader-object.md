...
ms.locfileid: "120089715"
---
# <a name="reader-object"></a>Objeto del lector

El objeto lector lee ejemplos de datos de archivos multimedia. Actualmente, el objeto reader admite archivos mediante la estructura de archivos de formato de sistemas avanzados (ASF), así como archivos MP3. Los datos entregados por el objeto lector están descomprimidos y listos para representarse de forma predeterminada, aunque las muestras se pueden entregar sin descomprimir si se desea. Los ejemplos se entregan de forma asincrónica desde el objeto de lector; debe configurar una función de devolución de llamada para recibirlas. Para la reproducción sincrónica de archivos ASF, use el objeto de lector sincrónico. Ni el lector ni el lector sincrónico representan datos. Debe proporcionar sus propias rutinas de representación para mostrar los medios recuperados de un archivo.
...
El objeto reader se crea mediante la función [**WMCreateReader**](/previous-versions/windows/desktop/api/Wmsdkidl/nf-wmsdkidl-wmcreatereader), que establece un puntero a una **interfaz IWMReader.** Las demás interfaces del objeto reader se pueden obtener llamando al **método QueryInterface.**
...
| [**IWMDRMReader**](/previous-versions/windows/desktop/api/wmsdkidl/nn-wmsdkidl-iwmdrmreader)                         | Administra la adquisición de licencias, las [*propiedades DRM*](wmformat-glossary.md) y la individualización del cliente.                                  |
...
| [**IWMReaderAdvanced**](/previous-versions/windows/desktop/api/wmsdkidl/nn-wmsdkidl-iwmreaderadvanced)               | Proporciona características avanzadas del lector, como un reloj proporcionado por el usuario, la asignación de búfer, estadísticas de devolución y notificaciones de selección de secuencias.                              |
