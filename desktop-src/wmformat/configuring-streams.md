...
1.  Obtenga un objeto de configuración de secuencia para la secuencia.
    -   Si va a crear una secuencia mediante uno de los códecs multimedia de Windows, debe obtener el objeto de configuración de secuencia como formato de códec mediante los métodos de [**IWMCodecInfo3**](/previous-versions/windows/desktop/api/wmsdkidl/nn-wmsdkidl-iwmcodecinfo3).
    -   Si la secuencia es un tipo arbitrario, obtenga un objeto de configuración de flujo vacío [**mediante IWMProfile::CreateNewStream**](/previous-versions/windows/desktop/api/Wmsdkidl/nf-wmsdkidl-iwmprofile-createnewstream).
2.  Configure la secuencia para satisfacer sus necesidades.
    -   Secuencias a todos los tipos se les debe asignar un nombre, un nombre de conexión y un número de secuencia.
    -   Secuencias usar Windows códecs multimedia solo se deben modificar de maneras predefinidas a partir del formato de códec. En el caso de las secuencias de audio, solo se debe cambiar la configuración de velocidad de bits variable (VBR) para VBR de dos pases. Las secuencias de vídeo deben configurarse con las propiedades de fotograma deseadas.
...
| Sección                                                                                                          | Descripción                                                                                                                                                     |
|------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Configuración común a todas las Secuencias](configuration-common-to-all-streams.md)                                   | Describe la configuración de flujo básica común a todos los tipos de secuencias.                                                                                        |
| [Obtención de información de configuración de secuencias de códecs](getting-stream-configuration-information-from-codecs.md) | Describe cómo obtener información de configuración de secuencias de los códecs para garantizar una configuración adecuada de las secuencias mediante los códecs multimedia de audio y vídeo Windows de streaming. |
| [Configuración de audio Secuencias](configuring-audio-streams.md)                                                       | Describe cómo configurar secuencias de audio.                                                                                                                       |
| [Configuración de vídeo Secuencias](configuring-video-streams.md)                                                       | Describe cómo configurar secuencias de vídeo.                                                                                                                       |
