...
ms.locfileid: "120033935"
---
# <a name="ctrpp"></a>CTRPP

La herramienta CTRPP es un preprocesador que analiza y valida el manifiesto para el proveedor V2. La herramienta genera recursos con las cadenas necesarias para los consumidores del proveedor y genera un encabezado con código que se usa para proporcionar los datos `.rc` `.h` del contador. Debe ejecutar la herramienta CTRPP durante la compilación del proveedor. Debe usar el código generado como punto de partida al desarrollar el proveedor en lugar de intentar generar este código usted mismo.
...
|**-ch** *symFile*   |Especifica el nombre del archivo de símbolos opcional que va `.h` a generar CTRPP. Este archivo contendrá símbolos de C/C++ para los nombres y GUID de cada conjunto de contadores del proveedor.
...
La herramienta CTRPP generará un archivo de recursos que contiene las cadenas localizables necesarias para los consumidores de los conjuntos `.rc` de contadores del proveedor.
...
- El desarrollador edita el archivo de manifiesto de proveedor ( ) para establecer el atributo del proveedor en el nombre de un binario de proveedor `.man` (.DLL, .SYS o .EXE) que contendrá los recursos de cadena para el proveedor y se instalará como parte del componente `applicationIdentity` del proveedor.
...
- Durante la instalación, el binario del proveedor se copia en el sistema del usuario y el manifiesto del proveedor se registra mediante [la herramienta lodctr](/windows-server/administration/windows-commands/lodctr). La herramienta lodctr convierte el atributo del manifiesto del proveedor en una ruta de acceso completa y registra la ruta de acceso completa al binario del `applicationIdentity` proveedor en el Registro.
  ...
  - Si el archivo binario usa LAV para la localización, asegúrese de copiar el archivo DEG junto con el binario.
...
- Variable global ***provider** _ que almacena el identificador de proveedor abierto por la función _ *_prefix_CounterInitialize**. El nombre de la variable es el valor del `symbol` atributo del elemento en el `provider` manifiesto. Esta variable debe usarse en llamadas a , y otras `PerfCreateInstance` API para controlar los datos del `PerfDeleteInstance` proveedor.
...
> [!NOTE]
> La función <b> *Add**Counterset del*</b> prefijo generado se usa cuando se tiene una devolución de llamada de conjunto de contadores. Las funciones <b> *Create**Counterset y*</b> <b> *Prefix* Close *Counterset*</b> generadas se usan cuando no se tiene una devolución de llamada de conjunto de contadores.

### <a name="using-the-generated-symbol-file"></a>Uso del archivo de símbolos generado

Si se **especifica el parámetro -ch** en la línea de comandos, la herramienta CTRPP generará un archivo `.h` de símbolos. Este archivo contiene los símbolos de C/C++ para los nombres y GUID de cada contraconjunto del proveedor. Los símbolos se pueden usar al escribir programas codificados de forma fuerte para consumir los datos de este contraconjunto mediante las funciones [consumidoras de PerfLib V2](using-the-perflib-functions-to-consume-counter-data.md).
