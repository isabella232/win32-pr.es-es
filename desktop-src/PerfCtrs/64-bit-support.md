...
ms.locfileid: "119011433"
---
# <a name="64-bit-support"></a>Compatibilidad con 64 bits

Un archivo DLL de proveedor de datos de rendimiento de 64 bits no se puede ejecutar en un proceso de consumidor de 32 bits y un archivo DLL de proveedor de datos de rendimiento de 32 bits no se puede ejecutar en un proceso de 64 bits. El registro del proveedor solo admite un valor único para la ruta de acceso al archivo DLL del proveedor de datos de rendimiento, por lo que no puede proporcionar rutas de acceso diferentes para que las usen consumidores de 32 bits y consumidores de `Library` 64 bits.
...
> [!NOTE]
> Varios proveedores de datos de rendimiento Windows integrados instalan un archivo DLL de 32 bits en , instalan un archivo DLL de 64 bits en y registran la ruta de acceso como , lo que permite que el redireccionamiento del sistema de archivos resuelva la ruta de acceso al `%systemroot%\syswow64` archivo `%systemroot%\system32` DLL `Library` `%systemroot%\system32\ProviderName.dll` adecuado. Esto solo se admite para los proveedores de datos de rendimiento que forman parte del Windows operativo. Los proveedores que no forman parte de Windows sistema operativo no deben instalar archivos en la `Windows` carpeta . Los archivos no reconocidos de la `Windows` carpeta se pueden quitar durante el mantenimiento o la actualización.
