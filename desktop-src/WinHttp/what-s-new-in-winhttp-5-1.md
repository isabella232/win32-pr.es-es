...
ms.locfileid: "127568168"
---
# <a name="whats-new-in-winhttp-51"></a>Novedades de WinHTTP 5.1

En este tema se describen las diferencias más importantes entre la versión 5.1 y la versión 5.0 de WinHTTP. Muchas de estas diferencias requieren cambios de código en las aplicaciones que migran de la versión 5.0 a la versión 5.1. Algunas de las características de la versión 5.1 solo están disponibles a partir de Windows Server 2003 y Windows XP con Service Pack 2 (SP2), especialmente características relacionadas con la mejora de la seguridad del cliente en servidores web malintencionados.

> [!IMPORTANT]
> Con la versión 5.1 de WinHTTP, la descarga de WinHTTP 5.0 ya no está disponible. A partir del 1 de octubre de 2004, Microsoft ha quitado la descarga del SDK de WinHTTP 5.0 de MSDN y ha finalizado la compatibilidad con el producto para la versión 5.0.
...
-   Cambio de comportamiento de redirección: las redirecciones HTTP desde una dirección URL **https:** segura a una **dirección HTTP normal:** la dirección URL ya no se siguen automáticamente de forma predeterminada por motivos de seguridad. Hay una nueva opción, **WINHTTP \_ OPTION REDIRECT \_ \_ POLICY**, para invalidar el comportamiento de redirección predeterminado en WinHTTP 5.1. Con el componente COM **WinHttpRequest,** use la nueva **opción \_ EnableHttpRequestOption EnableHttpsToHttpRedirects** para habilitar las redirecciones de https: a http: DIRECCIONES URL.
-   Cuando se crea un archivo de seguimiento WinHTTP, se restringe el acceso con una ACL de modo que solo los administradores puedan leer o escribir el archivo. La cuenta de usuario con la que se creó el archivo de seguimiento también puede modificar la ACL para conceder acceso a otros usuarios. Esta protección solo está disponible en sistemas de archivos que admiten la seguridad; es decir, NTFS, no FAT32).
-   A partir de Windows Server 2003 y Windows XP con SP2, el envío de solicitudes a los siguientes puertos conocidos que no son HTTP está restringido por motivos de seguridad: 21 (FTP), 25 (SMTP), 70 (GOPHER), 110 (POP3), 119 (NNTP), 143 (IMAP).
-   A partir de Windows Server 2003 y Windows XP con SP2, la cantidad máxima de datos de encabezado que WinHTTP acepta en una respuesta HTTP es 64K, de forma predeterminada. Si la respuesta HTTP del servidor contiene más de 64 000 datos de encabezado total, WinHTTP produce un error en la solicitud con **un error \_ WINHTTP INVALID SERVER \_ \_ \_ RESPONSE.** Este límite de 64 000 se puede invalidar mediante la nueva opción **WINHTTP \_ OPTION MAX RESPONSE HEADER \_ \_ \_ \_ SIZE.**
...
A partir Windows Server 2003 y Windows XP con SP2, WinHTTP 5.1 implementa las siguientes opciones nuevas. En Windows 2000 Professional con SP3 o Windows XP con SP1, las llamadas a [**WinHttpSetOption**](/windows/desktop/api/Winhttp/nf-winhttp-winhttpsetoption) o [**WinHttpQueryOption**](/windows/desktop/api/Winhttp/nf-winhttp-winhttpqueryoption) con estos IDs de opción producirán un error:
