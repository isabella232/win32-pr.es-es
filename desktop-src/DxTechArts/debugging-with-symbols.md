...
Hay varios tipos diferentes de símbolos disponibles para la depuración. Incluyen símbolos CodeView, COFF, DBG, SYM, PDB e incluso los símbolos de exportación que se generan a partir de una tabla de exportación de archivos binarios. En este documento técnico solo se VS.NET y los símbolos de formato PDB, ya que son el formato preferido más reciente. Se generan de forma predeterminada para los proyectos que se compilan mediante Visual Studio.

La generación de archivos PDB para archivos ejecutables de versión no afecta a las optimizaciones ni modifica significativamente el tamaño de los archivos generados. Normalmente, la única diferencia es la ruta de acceso y el nombre de archivo del archivo PDB se incrusta en el archivo ejecutable. Por esta razón, siempre debe generar archivos PDB, incluso si no desea enviarlos con el archivo ejecutable.
...
-   Lista de archivos de objeto que son responsables de secciones de código en el archivo ejecutable
...
-   Información sobre el archivo de origen y el número de línea
...
-   ARCHIVOS PDU que pueden estar presentes en la misma carpeta que el archivo DLL o ejecutable.
...
Para asegurarse de que tiene todos los archivos PB que necesita para una depuración precisa, instale las herramientas de depuración para Windows. Las versiones de 32 y 64 bits se pueden encontrar en Herramientas de depuración [para Windows](/windows-hardware/drivers/debugger/).

Una herramienta útil que se instala con este paquete es symchk.exe. Puede ayudar a identificar símbolos que faltan o son incorrectos. Esta herramienta tiene un gran número de posibles opciones de línea de comandos. Estas son dos de las más útiles y usadas con frecuencia.
...
/s **.** La opción **indica a symchk** que busque símbolos solo en la carpeta actual y que no busque en ningún servidor de símbolos.
...
La **opción /r** establece **symchk** para recorrer de forma recursiva las carpetas para comprobar que todos los archivos ejecutables tienen archivos PDU correspondientes. Sin la **opción /s,** **symchk** usa la ruta de acceso actual de SÍMBOLO DE NT para buscar símbolos en cualquier servidor local o privado, o en los servidores de símbolos \_ de \_ \_ Microsoft. La **herramienta symchk** solo busca símbolos para archivos ejecutables (.exe, .dll y similares). No puede usar caracteres comodín para buscar símbolos para archivos no ejecutables.

### <a name="how-symchk-works"></a>Funcionamiento de symchk

Cuando el vinculador genera archivos .dll, ejecutables y PDB, almacena GUID idénticos en cada archivo. Las herramientas usan el GUID para determinar si un archivo PDB determinado coincide con un archivo DLL o un archivo ejecutable. Si modifica un archivo DLL o ejecutable (mediante un editor de recursos o la codificación de protección de copia o modificando su información de versión), el GUID se actualiza y el depurador no puede cargar el archivo PDB. Por este motivo, es muy importante evitar manipular el archivo DLL o ejecutable después de que lo cree el vinculador.
...
-   Use la variable de entorno \_ NT \_ SYMBOL \_ PATH. Se recomienda este método.
    ...
    Lo usan todas las herramientas de depuración. También lo usa el Visual Studio y se lee y descodifica cuando se Visual Studio. Por lo tanto, si lo cambia, debe reiniciar Visual Studio.
...
Reemplace la caché local por el nombre de un directorio en el equipo en el que desea almacenar una memoria caché de los símbolos usados, por \[ \] ejemplo, símbolos %SYSTEMROOT% o \\ símbolos \\ c:.
...
Puede encontrar otras opciones para NT SYMBOL PATH en el archivo de ayuda que se instala con Las herramientas de depuración de \_ Microsoft para Windows \_ \_ paquete.

Los archivos ejecutables sin símbolos pueden aumentar el tiempo necesario para iniciar un depurador si se usa un servidor de símbolos. Esto se debe a que el depurador consulta el servidor de símbolos cada vez que intenta cargar el ejecutable. Por este motivo, es mejor solicitar siempre símbolos para todos los componentes.
...
Si ha configurado el depurador correctamente, carga automáticamente los símbolos que necesita de la memoria caché local o de un servidor de símbolos. Si desea obtener los símbolos para un solo archivo ejecutable o para una carpeta de ejecutables, puede usar **symchk**. Por ejemplo, si desea descargar los símbolos del archivo d3dx930.dll de la carpeta Windows System en el directorio actual, puede usar \_ el siguiente comando:

``` syntax
"c:\Program Files\Debugging Tools for Windows\symchk" c:\Windows\System32\d3dx9_30.dll /oc \.
```

La **herramienta symchk** tiene muchos otros usos. Para más información, **consulte symchk /?** o busque en la documentación de Microsoft Debugging Tools for Windows (Herramientas de depuración de Microsoft para Windows datos).
...
Para agregar, eliminar o editar archivos en un recurso compartido de servidor de símbolos, use symstore.exe herramienta. Esta herramienta forma parte de las Herramientas de depuración de Microsoft Windows paquete. La documentación completa sobre los servidores de símbolos, la herramienta symstore y los símbolos de indexación se incluye en herramientas de depuración para Windows paquete.
