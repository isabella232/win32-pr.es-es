...
Una aplicación recibe un evento de dispositivo [DBT \_ DEVICEQUERYREMOVE](dbt-devicequeryremove.md) cuando una característica del sistema ha decidido quitar un dispositivo especificado. Cuando la aplicación recibe este evento, debe determinar si está utilizando el dispositivo especificado y cancelar o preparar la eliminación.

En el ejemplo siguiente, una aplicación mantiene un identificador abierto, hFile, en el archivo o dispositivo representado por FileName. La aplicación se registra para la notificación de eventos de dispositivo en el dispositivo subyacente mediante una llamada a la función [**RegisterDeviceNotification**](/windows/desktop/api/Winuser/nf-winuser-registerdevicenotificationa) , mediante un filtro de notificación de tipo **DBT \_ DEVTYP \_ Handle** y especificando la variable hFile en el miembro de **\_ identificador dbch** del filtro.

La aplicación procesa el evento de dispositivo [DBT \_ DEVICEQUERYREMOVE](dbt-devicequeryremove.md) cerrando el identificador de archivo abierto en el dispositivo que se va a quitar. En el caso de que se cancele la eliminación de este dispositivo, la aplicación procesa el evento de dispositivo [DBT \_ DEVICEQUERYREMOVEFAILED](dbt-devicequeryremovefailed.md) para volver a abrir el identificador del dispositivo. Una vez que el dispositivo se ha quitado del sistema, la aplicación procesa los eventos de dispositivo [DBT \_ DEVICEREMOVECOMPLETE](dbt-deviceremovecomplete.md) y [DBT \_ DEVICEREMOVEPENDING](dbt-deviceremovepending.md) al anular el registro de su identificador de notificación para el dispositivo y cerrar los identificadores que todavía están abiertos en el dispositivo.
