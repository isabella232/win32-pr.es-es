---
title: Especificación de las credenciales de autenticación del servidor para un trabajo de transferencia de BITS
description: Puede especificar esquemas de autenticación diferentes para un Servicio de transferencia inteligente en segundo plano de transferencia de datos (BITS).
...
ms.locfileid: "119701825"
---
# <a name="specify-server-authentication-credentials-for-a-bits-transfer-job"></a>Especificación de las credenciales de autenticación del servidor para un trabajo de transferencia de BITS

Puede especificar esquemas de autenticación diferentes para un Servicio de transferencia inteligente en segundo plano de transferencia de datos (BITS).

El procedimiento siguiente obtiene un esquema de autenticación y crea una estructura de identidad de autenticación. A continuación, la aplicación crea un trabajo de descarga de BITS y establece las credenciales del trabajo. Una vez creado el trabajo, la aplicación obtiene un puntero a una interfaz de devolución de llamada y sondea el estado del trabajo de transferencia de BITS. Una vez completado el trabajo, se quita de la cola.
...
1.  Cree una estructura de contenedor que asigna valores [**BG \_ AUTH \_ SCHEME**](/windows/desktop/api/Bits1_5/ne-bits1_5-bg_auth_scheme) a nombres de esquema.
...
5.  Rellene la [**estructura BG \_ AUTH \_ CREDENTIALS**](/windows/desktop/api/Bits1_5/ns-bits1_5-bg_auth_credentials) con el esquema de autenticación devuelto por la función GetScheme y el nombre de usuario y la contraseña que se pasaron a la función ServerAuthentication.
6.  Obtenga un puntero a la [**interfaz IBackgroundCopyJob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) (pJob).
7.  Inicialice la seguridad del proceso COM mediante [una llamada a CoInitializeSecurity](/windows/win32/api/combaseapi/nf-combaseapi-coinitializesecurity). BITS requiere al menos el nivel IMPERSONATE de suplantación. BITS produce un error con E ACCESSDENIED si no se establece el nivel de \_ suplantación correcto.
8.  Obtenga un puntero a la [**interfaz IBackgroundCopyManager**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopymanager) y obtenga el localizador inicial de BITS mediante una llamada a la [función CoCreateInstance.]( /windows/win32/api/combaseapi/nf-combaseapi-cocreateinstance)
...
10. Obtenga un puntero a la interfaz de devolución de llamada CNotifyInterface y llame al método [**IBackgroundCopyJob::SetNotifyInterface**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-setnotifyinterface) para recibir notificaciones de eventos relacionados con el trabajo. Para obtener más información sobre CNotifyInterface, vea [Ejemplo: Clases comunes](common-classes.md).
11. Llame al [**método IBackgroundCopyJob::SetNotifyFlags**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-setnotifyflags) para establecer los tipos de notificaciones que se recibirán. En este ejemplo, se establecen **las marcas BG NOTIFY JOB \_ \_ \_ TRANSFERRED** y **BG NOTIFY JOB \_ \_ \_ ERROR.**
12. Obtenga un puntero a la [**interfaz IBackgroundCopyJob2.**](/windows/desktop/api/Bits1_5/nn-bits1_5-ibackgroundcopyjob2) Use el **puntero IBackgroundCopyJob2** para establecer propiedades adicionales en el trabajo. Este programa usa el [**método IBackgroundCopyJob2::SetCredentials**](/windows/desktop/api/Bits1_5/nf-bits1_5-ibackgroundcopyjob2-setcredentials) para establecer las credenciales del trabajo de transferencia de BITS.
13. Agregue archivos al trabajo de transferencia de BITS mediante una [**llamada a IBackgroundCopyJob::AddFile**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopyjob-addfile). En este ejemplo, el método **IBackgroundCopyJob::AddFile** usa las variables remoteFile y localFile que se pasaron a la función ServerAuthentication.
...
15. Configure un bucle while para esperar el mensaje de salida de la interfaz de devolución de llamada mientras se transfiere el trabajo.
    ...
    > [!Note]  
    > Este paso solo es necesario si el apartamento COM es un apartamento de un solo subproceso. Para obtener más información, [vea Single-Threaded Departamentos](../com/single-threaded-apartments.md).
    ...
    El bucle anterior usa la [función GetTickCount](/windows/win32/api/sysinfoapi/nf-sysinfoapi-gettickcount) para recuperar el número de milisegundos transcurridos desde que el trabajo empezó a transferirse.
