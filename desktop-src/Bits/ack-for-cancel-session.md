...
---
# <a name="ack-for-cancel-session"></a>Confirmación para Cancel-Session

Use el paquete **ACK para cancelar sesión** para confirmar la solicitud de [**cancelación de sesión**](cancel-session.md) del cliente. El servidor envía la confirmación después de liberar todos los recursos asociados a la sesión de carga.
...
GUID de cadena que identifica la sesión en el cliente. Reemplace {GUID} por el identificador de sesión que el cliente envió en el paquete de solicitud [**de cancelación de sesión**](cancel-session.md) . Si no reconoce el identificador de sesión, establezca el encabezado BITS-error-código en la sesión de \_ BG \_ E \_ no \_ encontrada.
