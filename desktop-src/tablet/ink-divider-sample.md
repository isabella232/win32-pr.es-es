...
---
# <a name="ink-divider-sample"></a>Ejemplo de divisor de tinta

Este ejemplo se basa en el [ejemplo de colección de entradas de lápiz](ink-collection-sample.md). Muestra cómo utilizar el objeto [divisor](/previous-versions/ms839398(v=msdn.10)) para analizar la entrada manuscrita.
...
Cuando se actualiza el formulario, el ejemplo dibuja un rectángulo delimitador alrededor de cada unidad analizada, dividida en palabras, líneas, párrafos y dibujos. Además de usar diferentes colores, estos rectángulos se amplían en diferentes cantidades para asegurarse de que ninguno de los rectángulos esté oculto por otros. En la tabla siguiente se especifica el color y la ampliación de cada unidad analizada.
...
Cuando se carga el formulario, se crea un objeto de [divisor](/previous-versions/ms839398(v=msdn.10)) . Se crea un objeto [InkOverlay](/previous-versions/ms833057(v=msdn.10)) y se asocia a un panel del formulario. A continuación, los controladores de eventos se adjuntan al objeto InkOverlay para realizar el seguimiento cuando se agregan y eliminan trazos. Si los reconocedores están disponibles, se asignará un objeto [RecognizerContext](/previous-versions/ms828542(v=msdn.10)) para el reconocedor predeterminado al divisor. A continuación, se establece la propiedad [LineHeight](/previous-versions/ms839409(v=msdn.10)) del objeto divisor y la colección [Strokes](/previous-versions/ms827799(v=msdn.10)) del objeto InkOverlay se asigna al divisor. Por último, el objeto InkOverlay está habilitado.
...
La colección de [trazos](/previous-versions/ms839422(v=msdn.10)) del objeto [divisor](/previous-versions/ms839398(v=msdn.10)) debe mantenerse sincronizada con la colección [Strokes](/previous-versions/ms827799(v=msdn.10)) del objeto [InkOverlay](/previous-versions/ms833057(v=msdn.10)) (a la que se tiene acceso a través de la propiedad [Ink](/previous-versions/ms833110(v=msdn.10)) del objeto InkOverlay). Para asegurarse de que esto ocurre, el controlador de eventos [Stroke](/previous-versions/ms835344(v=msdn.10)) para el objeto InkOverlay se escribe de la manera siguiente. Tenga en cuenta que el controlador de eventos primero comprueba si [EditingMode](/previous-versions/ms833105(v=msdn.10)) está establecido en **Ink** para filtrar los trazos de borrador. Si el usuario ha solicitado el análisis de diseño automático, la aplicación llama al método DivideInk del formulario y actualiza el área de dibujo.
...
Cuando el usuario hace clic en dividir en el menú Archivo, se llama al método [divide](/previous-versions/ms839461(v=msdn.10)) en el objeto [divisor](/previous-versions/ms839398(v=msdn.10)) . Si está disponible, se utiliza el reconocedor predeterminado.
...
El objeto [DivisionResult](/previous-versions/ms839371(v=msdn.10)) resultante, al que hace referencia la variable `divResult` , se pasa a una función de utilidad, `getUnitBBBoxes()` . La función de utilidad devuelve una matriz de rectángulos para cualquier tipo de división solicitado: segmentos, líneas, párrafos o dibujos.
...
Por último, se fuerza el redibujo del panel formulario para que aparezcan los rectángulos delimitadores.
...
## <a name="ink-analysis-results"></a>Resultados del análisis de tinta

En la función de la utilidad, se consultan los resultados del objeto [DivisionResult](/previous-versions/ms839371(v=msdn.10)) mediante el método [ResultByType](/previous-versions/ms839388(v=msdn.10)) , en función del tipo de división solicitado por el autor de la llamada. El método ResultByType devuelve una colección [DivisionUnits](/previous-versions/ms837954(v=msdn.10)) . Cada [DivisionUnit](/previous-versions/ms837976(v=msdn.10)) de la colección representa un dibujo, un solo segmento de reconocimiento de escritura a mano, una línea de escritura a mano o un bloque de escritura a mano, dependiendo de lo que se especificó cuando se llamó a la función de utilidad.
...
Si hay al menos un [DivisionUnit](/previous-versions/ms837976(v=msdn.10)), se crea una matriz de rectángulos que contiene un rectángulo delimitador por unidad. (Los rectángulos se inflan mediante diferentes cantidades para cada tipo de unidad, mantenidas en la variable inflada, para evitar la superposición).
...
Cuando se fuerza el redibujo anterior, el código siguiente se ejecuta para pintar los rectángulos de selección para cada [DivisionUnit](/previous-versions/ms837976(v=msdn.10)) del formulario alrededor de la tinta.
...
El método [Dispose](/dotnet/api/system.windows.forms.form.dispose?view=netcore-3.1) del formulario desecha los objetos [InkOverlay](/previous-versions/ms833057(v=msdn.10)), [divisor](/previous-versions/ms839398(v=msdn.10))y [RecognizerContext](/previous-versions/ms828542(v=msdn.10)) y la colección [Strokes](/previous-versions/ms827799(v=msdn.10)) utilizada en el ejemplo.
