---
description: Obtenga información sobre las definiciones de opciones en el esquema PrintCapabilities. Este tema no es actual. Para obtener la información más reciente, vea La especificación del esquema de impresión.
...
2.  Los atributos de nombre de los dos elementos son idénticos (o ninguno contiene un atributo name).
...
Ahora considere el caso en el que el dispositivo tiene un atributo de configuración de dispositivo que no se corresponde con ninguna de las instancias de características estándar. En este caso, no puede confiar en las instancias estándar de Option para proporcionar la lista de elementos ScoredProperty en común. Al crear una instancia de ScoredProperty, el objetivo principal es diferenciar cada opción de las demás en la característica y describir por qué un usuario seleccionaría una opción sobre otra. La línea de base es caracterizar cada opción con un atributo de nombre único y ScoredProperty que contiene el atributo name se convierte en la que se usa para determinar los elementos en común.
...
Debe tenerse en cuenta que el esquema de impresión requiere que el conjunto de instancias scoredProperty, sus ubicaciones y los valores asignados a cada ScoredProperty en una opción deben permanecer constantes, independientemente de la configuración. Todo el concepto del esquema de impresión se basa en las instancias de option que tienen instancias de Property y ScoredProperty fijas y identificables que se comparten entre muchos dispositivos.
