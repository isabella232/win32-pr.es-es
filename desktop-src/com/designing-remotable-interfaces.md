...
Incluso si la interfaz nunca se usará fuera de proceso, se puede usar entre subprocesos. El peor problema para un archivo IDL no desactivado puede surgir para los servidores en proceso que no admiten varios servidores de [un solo subproceso](single-threaded-apartments.md)). Un servidor que no especifica un modelo de subprocesos tiene implícitamente un único subproceso. Todo lo marcado como de subproceso único se fuerza al subproceso que primero llamó [**a CoInitialize**](/windows/desktop/api/Objbase/nf-objbase-coinitialize) o [**CoInitializeEx.**](/windows/desktop/api/combaseapi/nf-combaseapi-coinitializeex) Si algún otro subproceso fue el que activó el objeto , todas las interfaces de ese servidor de un solo subproceso se deben volver remotas al subproceso de activación, lo que puede dar lugar a una devolución de REGDB \_ E IIDNOTREG en respuesta a una llamada \_ a [**QueryInterface**](/windows/desktop/api/Unknwn/nf-unknwn-iunknown-queryinterface(q))). A menos que pueda aserr absolutamente que la interfaz está en proceso y que siempre se va a llamar en el mismo subproceso, se le remoto en algún momento.
