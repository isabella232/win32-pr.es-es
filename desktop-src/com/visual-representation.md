...
description: Un control admite el posicionamiento y la presentación dentro de su contenedor a través de la tecnología de documentos compuestos y la tecnología de arrastrar y colocar OLE que implica tanto el control como su contenedor.
...
Los controles pueden llamar a [**IOleControlSite::LockInPlaceActive**](/windows/desktop/api/OCIdl/nf-ocidl-iolecontrolsite-lockinplaceactive) en el objeto de sitio del contenedor para evitar que el contenedor intente degradar el control fuera del estado activo en el lugar. La degradación del control de esta manera hace que se desactive el control y se destruya su ventana, por lo que si el control debe mantener su ventana durante un tiempo conocido, puede llamar a **LockInPlaceActive** para garantizar su estado.
