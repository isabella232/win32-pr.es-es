...
Esto configura un valor inicial para el control deslizante. Se introdujeron dos nuevos mapas de bits. Uno es el mapa de bits de escala de grises (slider.bmp) que define los valores que se usarán cuando se haga clic en y el otro (slider.bmp) que determine la imagen que se mostrará cuando se haga clic en una parte determinada de la escala de grises.

El valor inicial se determina mediante la escucha del volumen con wmpprop y, a continuación, se puede cambiar el volumen cuando el usuario hace clic en una parte del control deslizante que desencadena un cambio de valor.
