---
title: PLAYLIST.disabledItemColor
description: El atributo disabledItemColor especifica o recupera el color de una pista de CD deshabilitada o del contenido en línea cuando está sin conexión.
ms.assetid: a6e3af4b-2492-43fe-b508-393632512971
keywords:
- PLAYLIST.disabledItemColor Reproductor de Windows Media
topic_type:
- apiref
api_name:
- PLAYLIST.disabledItemColor
api_type:
- NA
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: f1bf0d51cd921e3b8d164ffad3a26d20dc615bb0566dfaf5a97813e391108a72
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/11/2021
ms.locfileid: "118336788"
---
# <a name="playlistdisableditemcolor"></a>PLAYLIST.disabledItemColor

El **atributo disabledItemColor** especifica o recupera el color de una pista de CD deshabilitada o del contenido en línea cuando está sin conexión.

``` syntax
        elementID.disabledItemColor
```

## <a name="possible-values"></a>Valores posibles

Este atributo es una cadena de **lectura** y escritura que contiene cualquier valor de color Internet Explorer Microsoft. Tiene un valor predeterminado de "graytext".

## <a name="requirements"></a>Requisitos



| Requisito | Valor |
|--------------------|------------------------------------------------------|
| Versión<br/> | Reproductor de Windows Media versión 7.0 o posterior<br/> |



## <a name="see-also"></a>Consulte también

<dl> <dt>

[**Referencia de color**](color-reference.md)
</dt> <dt>

[**Elemento PLAYLIST**](playlist-element.md)
</dt> </dl>

 

 





