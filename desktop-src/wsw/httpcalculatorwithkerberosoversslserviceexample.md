...
En este ejemplo se muestra cómo usar el host de servicio para hospedar un servicio de calculadora a través de HTTP, con la seguridad de modo mixto de Kerberos a través de SSL. En esta configuración, la conexión de transporte está protegida (firmada, cifrada) por SSL, que también proporciona autenticación de servidor. La autenticación del cliente se proporciona mediante un vale APREQ de Kerberos en un encabezado WS-Security del mensaje.
