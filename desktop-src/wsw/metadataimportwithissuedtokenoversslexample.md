...
En este ejemplo se muestra la importación de metadatos de un punto de conexión que admite el uso de un token emitido que se obtiene del servicio de token de seguridad (STS) de emisión mediante [**WsRequestSecurityToken**](/windows/desktop/api/WebServices/nf-webservices-wsrequestsecuritytoken) y se presenta al servicio mediante el [**enlace de \_ seguridad de \_ \_ mensaje \_ \_ de token de WS XML**](/windows/desktop/api/WebServices/ns-webservices-ws_xml_token_message_security_binding) con el enlace de [**\_ \_ \_ seguridad \_ de transporte SSL de WS**](/windows/desktop/api/WebServices/ns-webservices-ws_ssl_transport_security_binding). Consulte [Federación](federation.md) para obtener más información.
