---
title: WS_POLICY (WebServices.h)
description: Tipo opaco que se usa para hacer referencia a una directiva de entrada de metadatos.
ms.assetid: 04623686-5065-4e97-8685-c72f848b92ab
keywords:
- WS_POLICY
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: 5f3daf0ac353e8d239ae03eb83e7a5dcbfe963754c52b90bf426566fc4ebae0f
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/11/2021
ms.locfileid: "120109685"
---
# <a name="ws_policy"></a>WS \_ POLICY

Tipo opaco que se usa para hacer referencia a una[directiva de entrada de](metadata-import.md) metadatos.


```C++
typedef struct _WS_POLICY WS_POLICY;
```



## <a name="remarks"></a>Comentarios

Este objeto no es seguro para subprocesos. Para obtener más información, vea [Seguridad para subprocesos.](thread-safety.md)

## <a name="requirements"></a>Requisitos



| Requisito | Valor |
|-------------------------------------|------------------------------------------------------------------------------------------|
| Cliente mínimo compatible<br/> | Windows 7 aplicaciones \[ de escritorio \| para UWP\]<br/>                                        |
| Servidor mínimo compatible<br/> | Windows Aplicaciones de escritorio para \[ UWP de Server 2008 R2 \|\]<br/>                           |
| Header<br/>                   | <dl> <dt>WebServices.h</dt> </dl> |



 

 





