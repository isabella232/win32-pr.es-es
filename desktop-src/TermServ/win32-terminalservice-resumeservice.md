...
El método de clase [WMI](/windows/desktop/WmiSdk/retrieving-a-class) **ResumeService** intenta colocar el servicio al que se hace referencia en estado reanudado.
...
Debe usar el método adecuado para iniciar un servicio que se ha detenido o para reanudar un servicio que se ha pausado. Los [**métodos de \_ servicio Win32**](/windows/desktop/CIMWin32Prov/win32-service) [**StartService**](win32-terminalservice-startservice.md) y **ResumeService** deben usarse en las situaciones siguientes:

-   Si un servicio está detenido actualmente, debe usar el [**método StartService**](win32-terminalservice-startservice.md) para reiniciarlo. **ResumeService** no puede iniciar un servicio que esté detenido actualmente.
...
El [ejemplo Resume AutoStart Services that are Paused](https://Gallery.TechNet.Microsoft.Com/413f2896-e7f3-4b3e-96cb-5abdc9bb6c36) VBScript (Reanudar servicios de inicio automático que están en pausa en VBScript) reinicia los servicios de inicio automático que se han pausado.
