---
description: Muestra la relación entre esos parámetros de función que apuntan a estructuras o matrices y sus datos inicializados, con un ejemplo de código.
...
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a><span data-ttu-id="642f8-103">Programa C de ejemplo: uso de CryptEncryptMessage y CryptDecryptMessage</span><span class="sxs-lookup"><span data-stu-id="642f8-103">Example C Program: Using CryptEncryptMessage and CryptDecryptMessage</span></span>

<span data-ttu-id="642f8-104">[**CryptEncryptMessage es**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) la única llamada de función necesaria para realizar todas las tareas enumeradas en [Cifrado de un mensaje.](../secauthn/encrypting-a-message.md)</span><span class="sxs-lookup"><span data-stu-id="642f8-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) is the only function call necessary to accomplish all of the tasks listed in [Encrypting a Message](../secauthn/encrypting-a-message.md).</span></span> <span data-ttu-id="642f8-105">Es necesaria la inicialización de estructuras de datos.</span><span class="sxs-lookup"><span data-stu-id="642f8-105">Initialization of data structures is necessary.</span></span> <span data-ttu-id="642f8-106">En la ilustración siguiente se muestra la relación entre los parámetros de función que apuntan a estructuras o matrices y sus datos inicializados.</span><span class="sxs-lookup"><span data-stu-id="642f8-106">The following illustration shows the relationship between those function parameters that point to structures or arrays and their initialized data.</span></span> <span data-ttu-id="642f8-107">En este ejemplo también se descifra el mensaje [**mediante CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span><span class="sxs-lookup"><span data-stu-id="642f8-107">This example also decrypts the message using [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span></span>

![mapa de inicialización para una llamada a cryptencryptmessage](images/crypenc.png)

<span data-ttu-id="642f8-109">**Para cifrar datos**</span><span class="sxs-lookup"><span data-stu-id="642f8-109">**To encrypt data**</span></span>

1.  <span data-ttu-id="642f8-110">Obtenga un puntero al contenido que se va a cifrar.</span><span class="sxs-lookup"><span data-stu-id="642f8-110">Get a pointer to the content to be encrypted.</span></span>
2.  <span data-ttu-id="642f8-111">Determine el tamaño del contenido que se va a cifrar.</span><span class="sxs-lookup"><span data-stu-id="642f8-111">Determine the size of the content to be encrypted.</span></span>
3.  <span data-ttu-id="642f8-112">Adquirir un identificador para un proveedor de servicios criptográficos.</span><span class="sxs-lookup"><span data-stu-id="642f8-112">Acquire a handle to a cryptographic provider.</span></span>
4.  <span data-ttu-id="642f8-113">Abra un [*almacén de certificados*](../secgloss/c-gly.md).</span><span class="sxs-lookup"><span data-stu-id="642f8-113">Open a [*certificate store*](../secgloss/c-gly.md).</span></span>
5.  <span data-ttu-id="642f8-114">Obtenga el certificado de destinatario.</span><span class="sxs-lookup"><span data-stu-id="642f8-114">Get the recipient certificate.</span></span>
6.  <span data-ttu-id="642f8-115">Cree la matriz de certificados de destinatario.</span><span class="sxs-lookup"><span data-stu-id="642f8-115">Create the recipient certificate array.</span></span>
7.  <span data-ttu-id="642f8-116">Inicialice la [**estructura CRYPT \_ ALGORITHM \_ IDENTIFIER.**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier)</span><span class="sxs-lookup"><span data-stu-id="642f8-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) structure.</span></span>
8.  <span data-ttu-id="642f8-117">Inicialice la [**estructura CRYPT \_ ENCRYPT MESSAGE \_ \_ PARA.**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para)</span><span class="sxs-lookup"><span data-stu-id="642f8-117">Initialize the [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) structure.</span></span>
9.  <span data-ttu-id="642f8-118">Llame [**a CryptEncryptMessage para**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) cifrar el contenido y crear un mensaje con sobre digital.</span><span class="sxs-lookup"><span data-stu-id="642f8-118">Call [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) to encrypt the content and create a digitally enveloped message.</span></span>

<span data-ttu-id="642f8-119">En el ejemplo siguiente se implementa este procedimiento.</span><span class="sxs-lookup"><span data-stu-id="642f8-119">The following example implements this procedure.</span></span>

<span data-ttu-id="642f8-120">Los comentarios relacionan fragmentos de código con cada paso del procedimiento.</span><span class="sxs-lookup"><span data-stu-id="642f8-120">Comments relate code fragments to each step in the procedure.</span></span> <span data-ttu-id="642f8-121">Para obtener más información sobre la función , vea [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span><span class="sxs-lookup"><span data-stu-id="642f8-121">For more information about the function, see [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span></span> <span data-ttu-id="642f8-122">Para obtener más información sobre las estructuras de datos, vea [**CRYPT \_ ALGORITHM \_ IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) y [**CRYPT \_ ENCRYPT MESSAGE \_ \_ PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span><span class="sxs-lookup"><span data-stu-id="642f8-122">For more information about the data structures, see [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span></span>

<span data-ttu-id="642f8-123">En este ejemplo se usan las [**funciones MyHandleError**](myhandleerror.md) [**y ByteToStr**](bytetostr.md).</span><span class="sxs-lookup"><span data-stu-id="642f8-123">This example uses the functions [**MyHandleError**](myhandleerror.md) and [**ByteToStr**](bytetostr.md).</span></span> <span data-ttu-id="642f8-124">El código de esta función se incluye con el ejemplo.</span><span class="sxs-lookup"><span data-stu-id="642f8-124">The code for this function is included with the sample.</span></span> <span data-ttu-id="642f8-125">El código de esta y otras funciones auxiliares también se muestra en [De uso general Functions](general-purpose-functions.md).</span><span class="sxs-lookup"><span data-stu-id="642f8-125">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
