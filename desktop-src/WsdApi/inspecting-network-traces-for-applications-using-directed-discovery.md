...
5.  Filtre los resultados para aislar el tráfico WS-Discovery de intercambio de metadatos. Para ver los filtros de Netmon de ejemplo, consulte [Descarga de netmon y filtros DPWS de ejemplo.](downloading-netmon-and-sample-dpws-filters.md)
    > [!Note]  
...
Los clientes y hosts de WSDAPI deben enviar mensajes que cumplan los criterios siguientes. Para obtener información general sobre los patrones de mensaje, vea Patrones de mensajes de intercambio de [metadatos y detección.](discovery-and-metadata-exchange-message-patterns.md)
...
-   Un [mensaje ProbeMatches](probematches-message.md) debe enviarse en un plazo de 4 segundos desde el mensaje [de sondeo](probe-message.md) correspondiente. El Firewall de Windows puede quitar un mensaje ProbeMatches enviado más de 4 segundos después de un mensaje de sondeo.
-   Si no se incluyó ningún elemento **XAddrs** en el mensaje [ProbeMatches](probematches-message.md) y el cliente o host enviará un mensaje HTTP (como una solicitud get [metadata](get--metadata-exchange--http-request-and-message.md) exchange o un mensaje de servicio), el cliente o host debe enviar un mensaje [Resolve](resolve-message.md) mediante HTTP o HTTPS. Este mensaje normalmente se envía al puerto 5357 o 5358.
...
-   Un [mensaje ResolveMatches](resolvematches-message.md) debe enviarse en un plazo de 4 segundos desde el mensaje [Resolve](resolve-message.md) correspondiente. El Firewall de Windows puede quitar un mensaje ResolveMatchesmessage enviado más de 4 segundos después de un mensaje resolver.
