...
| Nombre | Descripción |
|-|-|
| `float4()` | Crea un FLOAT4 no inicializado. |
| `float4(float x, float y, float z, float w)` | Crea un FLOAT4 con los valores especificados. |
| `float4(float2 value, float z, float w)` | Crea un FLOAT4 con x e y copiados de un float2 más los valores z y w especificados. |
| `float4(float3 value, float w)` | Crea un FLOAT4 con x, y y z copiados de un float3 más el valor de w especificado. |
| `explicit float4(float value)` | Crea un FLOAT4 con todos los com. entes establecidos en el valor especificado. |
| `float4(Microsoft::?Graphics::?Canvas::?Numerics::?Vector4 const& value)` | Convierte un **Microsoft. Graphics. Canvas. Numerics. Vector4** en FLOAT4. |
...
| `float length_squared(float4 const& value)` | Calcula la longitud, o euclidiana, del vector cuadrado. |
| `float distance(float4 const& value1, float4 const& value2)` | Calcula la distancia euclidiana entre dos vectores. |
| `float distance_squared(float4 const& value1, float4 const& value2)` | Calcula la distancia euclidiana entre dos vectores con un cuadrado. |
| `float dot(float4 const& vector1, float4 const& vector2)` | Calcula el producto escalar de dos vectores. |
...
| `float4 transform(float4 const& vector, float4x4 const& matrix)` | Transforma una FLOAT4 en la matriz especificada. |
| `float4 transform4(float3 const& position, float4x4 const& matrix)` | Transforma un float3 por la matriz especificada y devuelve una FLOAT4. |
| `float4 transform4(float2 const& position, float4x4 const& matrix)` | Transforma un float2 por la matriz especificada y devuelve una FLOAT4. |
| `float4 transform(float4 const& value, quaternion const& rotation)` | Transforma un FLOAT4 por el cuaternión especificado. |
| `float4 transform4(float3 const& value, quaternion const& rotation)` | Transforma un float3 por el cuaternión dado y devuelve una FLOAT4. |
| `float4 transform4(float2 const& value, quaternion const& rotation)` | Transforma un float2 por el cuaternión dado y devuelve una FLOAT4. |
...
| `static float4 unit_y()` | Devuelve la FLOAT4 (0, 1, 0, 0). |
| `static float4 unit_z()` | Devuelve el valor de FLOAT4 (0, 0, 1, 0). |
| `static float4 unit_w()` | Devuelve la FLOAT4 (0,0, 0, 0, 1). |
...
| `float4& operator*= (float4& value1, float value2)` | En contexto multiplica un vector por un valor escalar. |
...
| `operator Microsoft::?Graphics::?Canvas::?Numerics::?Vector4() const` | Convierte un FLOAT4 en un **Microsoft. Graphics. Canvas. Numerics. Vector4**. |
