...
La tabla atom global está disponible para todas las aplicaciones. Cuando una aplicación coloca una cadena en la tabla atom global, el sistema genera un atom que es único en todo el sistema. Cualquier aplicación que tenga el atom puede obtener la cadena que identifica consultando la tabla atom global.
...
1. Si hay preocupación sobre la aplicación que causa problemas con la tabla atom del usuario, puede investigar la causa principal conectando el depurador del kernel e irrumpiendo en el proceso en las llamadas a `UserAddAtomEx` ( `bae1 win32kbase!UserAddAtomEx /p <eprocess> "kc10;g"` ). Busque en `user32!` la pila de llamadas para ver a qué API se llama. La metodología es similar a la detección de problemas de tabla atom global que se explica [en Identificación de pérdidas](/archive/blogs/ntdebugging/identifying-global-atom-table-leaks)de tablas atom globales. Otra manera de volcar el contenido de la tabla atom del usuario es mediante una llamada a [GetClipboardFormatName](/windows/win32/api/winuser/nf-winuser-getclipboardformatnamea) en el intervalo de posibles 0xC000 a 0xFFFF. Si el recuento total de átomos aumenta constantemente mientras la aplicación se está ejecutando o no vuelve a la línea de base cuando se cierra la aplicación, hay un problema.
...
Varias funciones aceptan cadenas o atoms como parámetros. Al pasar un atom a estas funciones, una aplicación puede usar la macro [**MAKEINTATOM**](/windows/desktop/api/Winbase/nf-winbase-makeintatom) para convertir el atom en un formulario que pueda usar la función.
...
Los átomos enteros difieren de los de cadena de las maneras siguientes:

-   Los valores de los átomos enteros se encuentran en el intervalo 0x0001 a 0xBFFF **(DICONTATOM**– 1).
-   La representación de cadena de un atom entero es \# *dddd*, donde los valores representados por *dddd* son dígitos decimales. Se omiten los ceros iniciales.
-   No hay ningún recuento de referencias ni sobrecarga de almacenamiento asociada a un atom entero.
...
Una aplicación puede usar las funciones [**GetAtomName**](/windows/desktop/api/Winbase/nf-winbase-getatomnamea) o [**GlobalGetAtomName**](/windows/desktop/api/Winbase/nf-winbase-globalgetatomnamea) para recuperar una cadena de nombre de átomo de una tabla atom, siempre que la aplicación tenga el átomo correspondiente a la cadena que se busca. Ambas funciones copian la cadena de nombre de átomo del átomo especificado en un búfer y devuelven la longitud de la cadena que se copió. **GetAtomName recupera** una cadena de nombre de átomo de una tabla atom local y **GlobalGetAtomName** recupera una cadena de nombre de átomo de la tabla atom global.
...
| *nombre del átomo de cadena* | Nombre de átomo de cadena. Se usa para agregar un nombre de átomo de cadena a una tabla de átomos y recibir un átomo a cambio. |
