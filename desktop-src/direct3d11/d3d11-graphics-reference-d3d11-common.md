---
title: Referencia de versión común
description: La API de Direct3D define varios elementos de API que son comunes a Direct3D 12, Direct3D 11, Direct3D 10 y Direct3D 10.1.
ms.assetid: 2c9c354d-402f-4e79-9e54-ddf7e9b0c6db
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: e0d946c3e7e8c846b6cf33a805501a8c30bf0551d37155bd424ff97eecac65d5
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/11/2021
ms.locfileid: "118537886"
---
# <a name="common-version-reference"></a>Referencia de versión común

La API de Direct3D define varios elementos de API que son comunes a Direct3D 12, Direct3D 11, Direct3D 10 y Direct3D 10.1. Puede usar estos elementos de API en el código para cualquiera de estas versiones de Direct3D. Estos elementos de API se conocen como versión neutra.


## <a name="in-this-section"></a>En esta sección



| Tema                                                                                            | Descripción                                                                         |
|--------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|
| [Interfaces de versión comunes](d3d11-graphics-reference-d3d11-common-interfaces.md)<br/>     | Esta sección contiene información sobre las interfaces de versión comunes.<br/>   |
| [Estructuras de versión comunes](d3d11-graphics-reference-d3d11-common-structures.md)<br/>     | Esta sección contiene información sobre las estructuras de versión comunes.<br/>   |
| [Enumeraciones de versiones comunes](d3d11-graphics-reference-d3d11-common-enumerations.md)<br/> | Esta sección contiene información sobre las enumeraciones de versiones comunes.<br/> |



 

## <a name="related-topics"></a>Temas relacionados

<dl> <dt>

[Referencia de Direct3D 11](d3d11-graphics-reference.md)
</dt> </dl>

 

 





