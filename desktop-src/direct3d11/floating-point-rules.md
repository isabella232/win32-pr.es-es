...
description: Direct3D 11 admite varias representaciones de punto flotante. Todos los cálculos de punto flotante funcionan en un subconjunto definido de las reglas de punto flotante ieee 754 de precisión sencilla de 32 bits.
...
ms.locfileid: "119677115"
---
# <a name="floating-point-rules-direct3d-11"></a>Reglas de punto flotante (Direct3D 11)

Direct3D 11 admite varias representaciones de punto flotante. Todos los cálculos de punto flotante funcionan en un subconjunto definido de las reglas de punto flotante ieee 754 de precisión sencilla de 32 bits.
...
-   La división por 0 genera +/- INF, excepto 0/0, lo que da como resultado NaN.
-   El registro de (+/-) 0 genera -INF. El registro de un valor negativo (distinto de -0) genera NaN.
-   La raíz cuadrada recíproca (rsq) o la raíz cuadrada (sqrt) de un número negativo genera NaN. La excepción es -0; sqrt(-0) genera -0 y rsq(-0) genera -INF.
-   INF - INF = NaN
-   (+/-) INF / (+/-)INF = NaN
-   (+/-) INF \* 0 = NaN
-   NaN (cualquier valor op) = NaN
...
-   Las comparaciones de cualquier valor distinto de NaN con +/- INF devuelven el resultado correcto.
...
-   Las operaciones mínimas o máximas vacían los desnormados para la comparación, pero el resultado puede o no ser desnormado.
...
-   Las operaciones mínimas o máximas para las que solo un operando es NaN devuelven el otro operando como resultado (al contrario de las reglas de comparación que se han visto anteriormente). Se trata de una regla IEEE 754R.

    La especificación IEEE-754R para operaciones mínimas y máximas de punto flotante indica que si una de las entradas para min o max es un valor QNaN silencioso, el resultado de la operación es el otro parámetro. Por ejemplo:
...
-   Precisión: las operaciones sin fusionar en números de punto flotante de 16 bits generan un resultado que es el valor representable más cercano a un resultado infinitamente preciso (redondeado a par más cercano, por IEEE-754, aplicado a valores de 16 bits). Las reglas de punto flotante de 32 bits se adhieren a 1 tolerancia ULP, las reglas de punto flotante de 16 bits se adhieren a 0,5 ULP para operaciones sin fusionar y 0,6 ULP para operaciones fusionadas.
