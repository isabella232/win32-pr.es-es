...
Esta función devuelve el [JET_ERR](./jet-err.md) de datos con uno de los siguientes códigos de retorno. Para obtener más información sobre los errores de ESE, vea [Extensible Storage Engine Errors](./extensible-storage-engine-errors.md) and Error Handling [Parameters](./error-handling-parameters.md).
...
En caso de error, no se creará la tabla temporal que contiene los resultados. Se puede cambiar el estado de la base de datos temporal. El estado de las bases de datos normales en uso por el motor de base de datos permanecerá sin cambios. La posición actual de las [JET_TABLEID](./jet-tableid.md)proporcionadas a esta función puede cambiarse.
