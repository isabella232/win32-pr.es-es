...
**JetTerm2 también** puede destruir una instancia sin inicializar creada por [JetCreateInstance.](./jetcreateinstance-function.md)
...
|--------------|----------------|
| <p>JET_bitTermComplete</p> | <p>Solicita que la instancia se apague correctamente. Cualquier trabajo de limpieza opcional que normalmente se realizaría en segundo plano en tiempo de ejecución se completa inmediatamente.</p> | 
| <p>JET_bitTermAbrupt</p> | <p>Solicita que la instancia se apague lo más rápido posible. Se abandona cualquier trabajo opcional que normalmente se realizaría en segundo plano en tiempo de ejecución.</p><p><strong>Nota</strong>  Esta opción puede provocar una pérdida de espacio temporal o permanente en la base de datos. Este espacio perdido siempre se puede recuperar mediante una desfragmentación sin conexión de la base de datos.</p> | 
| <p>JET_bitTermStopBackup</p> | <p>Solicita que la instancia se apague incluso si actualmente hay una copia de seguridad en curso. Normalmente, una copia de seguridad pendiente provocaría un error <a href="gg269298(v=exchg.10).md">en JetTerm</a> con JET_errBackupInProgress. Cuando este parámetro no está presente, se supone que su valor JET_bitTermAbrupt.</p> | 
...
| <p>JET_errInvalidParameter</p> | <p>Uno de los parámetros proporcionados contenía un valor inesperado o la combinación de varios parámetros producía un resultado inesperado. <a href="gg269298(v=exchg.10).md">JetTerm</a> devolverá este error cuando el motor esté en modo de varias instancias y cuando <em>pinstance</em> haga referencia a una instancia no válida.</p><p><strong>Windows XP:</strong>  Este valor devuelto se introduce en Windows XP.</p> | 
...
Si se produce un error en esta función como resultado de un error de uso, la instancia permanece en estado inicializado y no cambia nada. De lo contrario, la instancia todavía se cierra como se indica para el caso de éxito. La diferencia es que la instancia tendrá que pasar por la recuperación de bloqueos cuando se inicialice a continuación. El motor intentará vaciar tantos datos como sea posible para minimizar la cantidad de recuperación necesaria. Conceptualmente, este error de [JetTerm](./jetterm-function.md) no es diferente de un bloqueo del proceso.
