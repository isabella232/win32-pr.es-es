...
ms.locfileid: "119998365"
---
# <a name="dynamic-format-changes"></a>Cambios de formato dinámico

Cuando se conectan dos filtros, se ponen de acuerdo en un tipo de medio, que describe el formato de los datos que el filtro ascendente entregará. En la mayoría de los casos, el tipo de medio se fija mientras dura la conexión. Sin embargo, DirectShow ofrece compatibilidad limitada con los filtros para cambiar el tipo de medio. Cuando un filtro cambia los tipos de medios, se denomina cambio *de formato dinámico.* Si va a escribir un filtro DirectShow, debe tener en cuenta los mecanismos para los cambios de formato dinámico. Incluso si el filtro no admite estos cambios, debe responder correctamente si otro filtro solicita un nuevo formato.
...
De lo contrario, si el gráfico está activo, pero los filtros en cuestión no admiten reconexiones de pin dinámicas, hay tres mecanismos posibles para cambiar el formato:

-   [QueryAccept (downstream)](queryaccept--downstream.md) se usa cuando si un pin de salida propone un cambio de formato en su elemento del mismo nivel de bajada, pero solo si el nuevo formato no requiere un búfer mayor.
-   [QueryAccept (upstream) se](queryaccept--upstream.md) usa cuando un pin de entrada propone un cambio de formato en su elemento del mismo nivel ascendente. El nuevo formato puede tener el mismo tamaño o puede ser mayor.
-   [ReceiveConnection](receiveconnection.md) se usa cuando un pin de salida propone un cambio de formato en su elemento del mismo nivel de bajada y el nuevo formato requiere un búfer mayor.
