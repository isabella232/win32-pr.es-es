...
Para reproducir un archivo Windows Media Audio multicanal en DirectShow, debe establecer la propiedad **MFPKEY \_ WMADEC \_ HIRESOUTPUT** directamente en el descodificador después de haberse conectado al lector ASF de WM. Esta propiedad se define en el archivo de encabezado wmcodecdsp. h, que está disponible en el Windows SDK.

> [!Note]  
> Este procedimiento de configuración solo se admite para los archivos que no están protegidos por Rights Management digital.
...
2.  Obtenga un puntero al filtro de contenedor de DMO.
3.  Desconecte el contenedor DMO del representador de audio.
4.  Use la interfaz **IPropertyBag** para establecer la **propiedad \_ \_ HIRESOUTPUT de MFPKEY WMADEC** en el descodificador. La constante global **g \_ wszWMACHiResOutput** define el nombre de la propiedad.
5.  Vuelva a conectar el contenedor DMO y el representador de audio.
6.  Ejecute el gráfico.

En los fragmentos de código siguientes se muestran estos pasos. En este código se supone que el archivo de origen contiene una secuencia de audio y ningún flujo de vídeo. El códec de vídeo DMO no admite la propiedad **MFPKEY \_ WMADEC \_ HIRESOUTPUT** .
