---
description: En este tema se describe cómo buscar un pin no relacionado en un filtro. La búsqueda de un pin no conectado resulta útil al conectar filtros.
ms.assetid: d0a906a8-bae4-43b3-8b02-ee5b97c9323d
title: Buscar un pin no desconectado en un filtro
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "127063272"
---
# <a name="find-an-unconnected-pin-on-a-filter"></a>Buscar un pin no desconectado en un filtro

En este tema se describe cómo buscar un pin no relacionado en un filtro. La búsqueda de un pin no conectado resulta útil al conectar filtros.

En un escenario DirectShow de creación de grafos, necesita un pin no desconectado que coincida con una dirección de pin determinada (entrada o salida). Por ejemplo, cuando se conectan dos filtros, se conecta un pin de salida de un filtro a un pin de entrada del otro filtro. Ambos pines deben estar desconectados antes de conectarlos.
...
A continuación, necesitamos una función que comprueba si un pin coincide con una dirección de patilla especificada. Esta función llama al [**método IPin::QueryDirection**](/windows/desktop/api/Strmif/nf-strmif-ipin-querydirection) para obtener la dirección del pin.
...
La siguiente función coincide con un pin según ambos criterios (dirección de la ancla y estado de conexión).
