...
Las aplicaciones pueden usar el modo de combinación de VMR para mostrar logotipos de canales alfa blended, una interfaz de usuario o anuncios parcial o completamente dentro del rectángulo de vídeo. Dado que el procesador de gráficos realiza la mezcla en hardware, el rendimiento de reproducción de Video Stream es mínimo y no hay artefactos detectables de parpadeo o desgarro. Las aplicaciones pueden cambiar la imagen mostrada con la frecuencia que quieran. Debe tenerse en cuenta que los cambios solo se reflejan en la pantalla cuando el DirectShow de filtro está en estado en ejecución.
...
Los conceptos que se deban tratar en este tema se muestran en la aplicación de ejemplo [VMRPlayer Sample.](vmrplayer-sample.md)

**Crear animaciones simples con la imagen de mapa de bits**

Para crear un logotipo de mapa de bits animado simple, coloque todos los "marcos" de mapa de bits en una sola imagen, como se muestra en la ilustración siguiente.

![vmr image strip](images/vmr-image-strip.png)

Al establecer el mapa de bits inicialmente mediante [**IVMRMixerBitmap::SetAlphaBitmap**](/windows/desktop/api/Strmif/nf-strmif-ivmrmixerbitmap-setalphabitmap), si el mapa de bits está en un HDC, establezca el **campo rSrc** de la estructura **VMRALPHABITMAP para** especificar el tamaño de todo el mapa de bits dentro de HDC. Los **miembros** superior **e izquierdo** de la estructura  se  establecen en 0, y los miembros derecho e inferior son el ancho y alto del mapa de bits. Si el mapa de bits está en una superficie de DirectDraw, se conoce el tamaño de la superficie, por lo que no es necesario especificar rSrc en este método.

Al llamar a [**IVMRMixerBitmap::UpdateAlphaBitmapParameters**](/windows/desktop/api/Strmif/nf-strmif-ivmrmixerbitmap-updatealphabitmapparameters), use el miembro **rSrc** para los mapas de bits HDC y DirectDraw, para especificar el marco o rectángulo concreto dentro de la imagen que desea mostrar y establezca la marca **\_ SRCRECT VMRBITMAP** en el **miembro dwFlags.**
