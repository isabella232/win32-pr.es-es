...
title: Lectura DRM-Protected archivos de ASF en DirectShow
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "122786980"
---
# <a name="reading-drm-protected-asf-files-in-directshow"></a>Lectura DRM-Protected archivos de ASF en DirectShow
...
-   El proveedor de contenido empaqueta el archivo, es decir, cifra el archivo y adjunta la información de licencia al encabezado de archivo ASF. La información de licencias incluye una dirección URL donde el cliente puede obtener la licencia.
...
Dado que El servicio de individualización de Microsoft controla la información del usuario, debe mostrar la directiva de privacidad de Microsoft o proporcionar un vínculo a esa página en el sitio web de Microsoft antes de que la aplicación se individualice: <https://go.microsoft.com/fwlink/p/?linkid=10240> .

## <a name="provide-the-software-certificate"></a>Proporcionar el certificado de software

Para permitir que la aplicación use la licencia DRM,  la aplicación debe proporcionar un certificado de software o una clave al Administrador de filtros Graph administrador. Esta clave está contenida en una biblioteca estática que se individualiza para la aplicación. Para obtener información sobre cómo obtener la biblioteca individualizada, consulte [Obtención](../wmformat/obtaining-the-required-drm-library.md) de la biblioteca DRM necesaria en la documentación Windows SDK de formato multimedia.
...
5.  El Administrador Graph filtro llamará **a IServiceProvider::QueryService** y especificará **\_ IID IWMReader** para el identificador de servicio.
...
El código siguiente muestra cómo llamar a [**SetSite**](/windows/win32/api/ocidl/nf-ocidl-iobjectwithsite-setsite) en el Administrador de Graph filtros:
...
## <a name="building-the-playback-graph"></a>Creación del archivo de reproducción Graph

Para reproducir un archivo ASF protegido con DRM, realice los pasos siguientes:

1.  Cree el [Administrador de Graph y](filter-graph-manager.md) use la interfaz [**IMediaEventEx**](/windows/desktop/api/Control/nn-control-imediaeventex) para registrarse en eventos de grafos.
...
7.  En el primer [**evento EC \_ WMT \_ EVENT,**](ec-wmt-event.md) consulte el filtro [LECTOR DE ASF de WM](wm-asf-reader-filter.md) para la interfaz **IServiceProvider.**
...
9.  Llame [**a IGraphBuilder::Render**](/windows/desktop/api/Strmif/nf-strmif-igraphbuilder-render) para representar las clavijas de salida del filtro [WM ASF Reader.](wm-asf-reader-filter.md)
...
> [!Note]  
> Es importante registrarse para los eventos de grafo (paso 1) antes de agregar el filtro [lector asf](wm-asf-reader-filter.md) de WM al gráfico (paso 3), ya que la aplicación debe controlar los eventos [**EC \_ WMT \_ EVENT.**](ec-wmt-event.md) Los eventos se envían cuando [**se llama**](/windows/desktop/api/Strmif/nf-strmif-ifilesourcefilter-load) a Load (paso 5).
...
En el código anterior, la función enumera las clavijas de salida en el filtro WM ASF Reader y llama a `RenderOutputPins` [**IGraphBuilder::Render**](/windows/desktop/api/Strmif/nf-strmif-igraphbuilder-render) para cada pin. [](wm-asf-reader-filter.md)
...
El código siguiente muestra cómo obtener un puntero a la [**interfaz IWMDRMReader**](/previous-versions/windows/desktop/api/wmsdkidl/nn-wmsdkidl-iwmdrmreader) desde el [lector DE ASF de WM](wm-asf-reader-filter.md):
