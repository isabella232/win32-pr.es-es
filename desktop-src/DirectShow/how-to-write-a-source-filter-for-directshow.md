...
Si no hay muestras disponibles, el paso 1 se bloquea hasta que una muestra esté disponible. El paso 4 también puede bloquearse; por ejemplo, puede bloquearse mientras el gráfico está en pausa.

El bucle se ejecuta lo más rápido posible, pero está limitado por la rapidez con la que el filtro del representador representa cada ejemplo. Suponiendo que el gráfico de filtros tenga un reloj de referencia, la velocidad viene determinada por los tiempos de presentación de los ejemplos. Si no hay ningún reloj de referencia, el representador consume muestras lo más rápido posible.
...
-   [**CSource**](csource.md) es la clase base del filtro e implementa la [**interfaz IBaseFilter.**](/windows/desktop/api/Strmif/nn-strmif-ibasefilter)
...
Un filtro de origen puede tener más de un pin de salida. En el método de constructor del filtro, cree uno o varios pines derivados de [**CSourceStream**](csourcestream.md) (un pin por cada flujo de salida). No es necesario almacenar punteros a los pines; los pines se agregan automáticamente al filtro cuando se crean.
...
| Método                                                 | Descripción                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|--------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**GetMediaType**](csourcestream-getmediatype.md)     | Obtiene un tipo de medio de la patilla de salida. <br/> El pin debe proponer al menos un tipo de medio, porque es posible que el filtro de nivel inferior no proponga ningún tipo. En la mayoría de los casos, el filtro de bajada será un descodificador o un representador, en función de si el filtro de origen entrega datos comprimidos o sin comprimir. Normalmente, un filtro de representador requiere un tipo de medio completo, que contiene toda la información de formato necesaria para representar la secuencia. Para un descodificador, la cantidad de información necesaria en el tipo de medio depende en gran medida del formato de codificación.<br/> |
| [**CheckMediaType**](csourcestream-checkmediatype.md) | Comprueba si la marca de salida acepta un tipo de medio determinado. La invalidación de este método es opcional, en función de cómo implemente [**GetMediaType.**](csourcestream-getmediatype.md)                                                                                                                                                                                                                                                                                                                                                                                                         |
...
En el caso de los formatos de vídeo sin comprimir, recuerde que el filtro de nivel inferior puede proponer formatos con varios valores de paso. El filtro debe aceptar cualquier valor de stride válido. Para obtener más información, vea [**BITMAPINFOHEADER**](/windows/win32/api/wingdi/ns-wingdi-bitmapinfoheader).
...
La clase base se inicia y detiene automáticamente el bucle de subprocesos en los momentos adecuados, cuando el filtro se detiene o se detiene. Cuando esto sucede, la [**clase CSourceStream**](csourcestream.md) llama a algunos métodos para notificar a la clase derivada:
...
Si tiene un filtro de origen con un pin de salida, puede usar la clase [**CSourceSeeking**](csourceseeking.md) como punto de partida para implementar la búsqueda. Herede la clase de pin de [**CSourceStream**](csourcestream.md) y **CSourceSeeking.**
...
La [**clase CSourceSeeking**](csourceseeking.md) administra la velocidad de reproducción, la hora de inicio, la hora de detenerse y la duración. La clase derivada debe establecer la duración y el tiempo de detenerse iniciales. Cada vez que cambia uno de estos valores, se llama al método [**CSourceSeeking::ChangeRate**](csourceseeking-changerate.md), [**CSourceSeeking::ChangeStart**](csourceseeking-changestart.md)o [**CSourceSeeking::ChangeStop,**](csourceseeking-changestop.md) según corresponda. Todos los métodos son métodos virtuales puros. La clase de pin derivada invalida estos métodos para hacer lo siguiente:

1.  Llame [**a IPin::BeginFlush en**](/windows/desktop/api/Strmif/nf-strmif-ipin-beginflush) el pin de nivel inferior. Esto hace que los filtros de nivel inferior liberen muestras que están manteniendo y rechacen nuevas muestras.
2.  Llame [**a CSourceStream::Stop para**](csourcestream-stop.md) detener el subproceso de streaming. El filtro de origen suspende la generación de nuevos datos.
3.  Llame [**a IPin::EndFlush en**](/windows/desktop/api/Strmif/nf-strmif-ipin-endflush) el pin de nivel inferior. Esto indica a los filtros de nivel inferior que acepten nuevos datos.
...
Para obtener más información, [vea Compatibilidad con la búsqueda en un filtro de origen.](supporting-seeking-in-a-source-filter.md)

Si el filtro admite búsquedas, la posición de la secuencia ahora es independiente del tiempo de presentación. Después de una búsqueda, las marcas de tiempo se restablecen a cero. La fórmula general para las marcas de tiempo es:

-   sample start time = time elapsed/playback rate
-   sample end time = sample start time + (time per frame/playback rate)

donde *transcurrió el* tiempo transcurrido desde que el filtro empezó a ejecutarse o desde el último comando de búsqueda.
...
Si la aplicación establece un nuevo formato de hora, todos los parámetros de posición de los métodos [**IMediaSeeking**](/windows/desktop/api/Strmif/nn-strmif-imediaseeking) se interpretan en términos del nuevo formato de hora. Por ejemplo, si el formato de tiempo es fotogramas, el método [**IMediaSeeking::GetDuration**](/windows/desktop/api/Strmif/nf-strmif-imediaseeking-getduration) debe devolver la duración en fotogramas.
